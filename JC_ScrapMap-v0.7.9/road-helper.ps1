...
$HookLine = @(
    ...
    'if not jc_loader_ok then pcall(sm.json.save, { protocol = "jc-scrapmap-hook-status-v1", stage = "loader-error", message = tostring(jc_loader_error) }, "$SURVIVAL_DATA/JC_ScrapMap/roads-hook-status.json") end'
) -join "`r`n"
