...
$HookLine = @(
    'sm.log.info("JC ScrapMap stage: hook-loaded")'
    'pcall(sm.json.save, { protocol = "jc-scrapmap-hook-status-v1", stage = "hook-loaded", message = "Survival terrain hook executed." }, "$SURVIVAL_DATA/JC_ScrapMap/roads-hook-status.json")'
    'local jc_loader_ok, jc_loader_error = pcall(dofile, "$SURVIVAL_DATA/Scripts/terrain/jc_scrapmap_road_export.lua")'
    'if not jc_loader_ok then sm.log.error("JC ScrapMap stage: loader-error: " .. tostring(jc_loader_error)); pcall(sm.json.save, { protocol = "jc-scrapmap-hook-status-v1", stage = "loader-error", message = tostring(jc_loader_error) }, "$SURVIVAL_DATA/JC_ScrapMap/roads-hook-status.json") end'
) -join "`r`n"
...
function Invoke-ElevatedAction([string]$RequestedAction, [switch]$NoWait) {
    $launchingUserSid = if ($DesktopUserSid) { $DesktopUserSid } else { Get-CurrentUserSid }
    $arguments = @(
        '-NoProfile',
        '-ExecutionPolicy', 'Bypass',
        '-File', "`"$PSCommandPath`"",
        '-Action', $RequestedAction,
        '-DesktopUserSid', $launchingUserSid,
        '-Port', $Port
    )
...
function Start-GenerateWindow {
    ...
    $arguments = @(
        ...
        '-File', "`"$PSCommandPath`"",
        '-Action', 'Generate',
        '-DesktopUserSid', (Get-CurrentUserSid),
        '-Port', $Port
    )
    if ($GamePath) { $arguments += @('-GamePath', "`"$GamePath`"") }
    if ($UserPath) { $arguments += @('-UserPath', "`"$UserPath`"") }
...
function Add-RecentGameEvidence(
    [DateTime]$SinceUtc,
    ...
    foreach ($log in $logs) {
        ...
        if ($log.Extension -in @('.log', '.txt') -and $log.Length -gt 0) {
            try {
                $markers = @(Select-String -LiteralPath $log.FullName -Pattern 'JC ScrapMap' -SimpleMatch -ErrorAction Stop |
                    Select-Object -Last 100)
                if ($markers.Count -eq 0) {
                    Add-Diagnostic 'GAMELOG' 'No JC ScrapMap stage marker was present in this recent game log.'
                }
                foreach ($marker in $markers) {
                    if ($marker.Line.Trim()) { Add-Diagnostic 'GAMELOG>' ([string]$marker.Line) }
                }
            } catch {
                Add-Diagnostic 'GAMELOG' "Could not inspect JC ScrapMap markers: $($_.Exception.Message)"
            }
        }
    }
...
function Get-Paths {
    ...
    return @{
        ...
        HookStatus = Join-Path $resolvedGame 'Survival\JC_ScrapMap\roads-hook-status.json'
    }
}
...
function Grant-OutputAccess([string]$Path, [string]$UserSid) {
    if (-not $UserSid) {
        throw 'The desktop user SID was not preserved across elevation.'
    }
    try {
        $sid = New-Object Security.Principal.SecurityIdentifier($UserSid)
    } catch {
        throw "The desktop user SID is invalid: $(Get-CompactError $_)"
    }
    ...
    $modifyMask = [int][Security.AccessControl.FileSystemRights]::Modify
    $sidRules = $verifiedAcl.GetAccessRules(
        $true,
        $true,
        [Security.Principal.SecurityIdentifier]
    )
    $verified = @($sidRules | Where-Object {
        $_.IdentityReference.Value -eq $sid.Value -and
        $_.AccessControlType -eq [Security.AccessControl.AccessControlType]::Allow -and
        (([int]$_.FileSystemRights -band $modifyMask) -eq $modifyMask) -and
        ($_.InheritanceFlags -band [Security.AccessControl.InheritanceFlags]::ContainerInherit) -and
        ($_.InheritanceFlags -band [Security.AccessControl.InheritanceFlags]::ObjectInherit)
    }).Count -gt 0
    if (-not $verified) {
        throw 'The temporary output ACL did not retain the required desktop-user Modify rule.'
    }
    return $sid.Value
}
...
function Enable-Helper {
    ...
    if (Test-Path -LiteralPath $paths.Output) {
        $existingOutput = @(Get-ChildItem -LiteralPath $paths.Output -Force)
        if ($existingOutput.Count -eq 0) {
            Remove-Item -LiteralPath $paths.Output -Force
            Add-Diagnostic 'RECOVERY' 'Removed an empty output directory left by an incomplete earlier activation.'
        } else {
            throw "Refusing to overwrite an existing non-empty terrain output directory: $($paths.Output)"
        }
    }
    ...
    @{
        ...
        hookStatusPath = $paths.HookStatus
    ...
    $outputUserSid = if ($DesktopUserSid) { $DesktopUserSid } else { Get-CurrentUserSid }
    $null = Grant-OutputAccess $paths.Output $outputUserSid
    Add-Diagnostic 'OUTPUT-ACL' 'Granted and verified inherited Modify access for the desktop user account.'
    ...
    if ([int]$state.schemaVersion -eq 1) {
        ...
        if (Test-Path -LiteralPath $state.outputPath -PathType Container) {
            if (-not (Test-Path -LiteralPath $state.ownerPath -PathType Leaf)) {
                if (@(Get-ChildItem -LiteralPath $state.outputPath -Force).Count -eq 0) {
                    Remove-Item -LiteralPath $state.outputPath -Force
                    Add-Diagnostic 'CLEANUP' 'Removed an empty temporary output directory left before ownership-marker creation.'
                } else {
                    throw 'Temporary output ownership marker is missing and the directory is not empty; refusing removal.'
                }
            } else {
                ...
                $managedPaths = @($state.exportPath, $state.statusPath, $state.hookStatusPath, $state.ownerPath) |
                    Where-Object { $_ } |
                ...
                foreach ($managedFile in @($state.exportPath, $state.statusPath, $state.hookStatusPath, $state.ownerPath)) {
...
function Generate-Roads {
    ...
    $lastHookStatusTimestamp = [DateTime]::MinValue
    $reportedStatusStages = New-Object 'System.Collections.Generic.HashSet[string]'
    $exportReady = $false
    $stableLength = [long]-1
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        ...
        if ($gameIsRunning) {
            ...
            if (Test-Path -LiteralPath $statusPath -PathType Leaf) {
                ...
                Add-Diagnostic 'EXPORTER' 'The exporter status file was never created. The report cannot infer whether the hook did not execute or the game process could not write its status.'
            }
            if (Test-Path -LiteralPath $hookStatusPath -PathType Leaf) {
                try {
                    Add-Diagnostic 'HOOK' "Final hook status: $((Get-Content -LiteralPath $hookStatusPath -Raw | ConvertFrom-Json | ConvertTo-Json -Compress -Depth 5))"
                } catch {
                    Add-Diagnostic 'HOOK' "Final hook status file was unreadable: $(Get-CompactError $_)"
                }
            } else {
                Add-Diagnostic 'HOOK' 'The separate terrain-hook sentinel was never created.'
            }
            Add-RecentGameEvidence $sessionStartedUtc $paths.User $paths.Game
        ...
        if (Test-Path -LiteralPath $hookStatusPath -PathType Leaf) {
            $hookStatusItem = Get-Item -LiteralPath $hookStatusPath
            if ($hookStatusItem.LastWriteTimeUtc -gt $lastHookStatusTimestamp) {
                $lastHookStatusTimestamp = $hookStatusItem.LastWriteTimeUtc
                try {
                    $hookStatus = Get-Content -LiteralPath $hookStatusPath -Raw | ConvertFrom-Json
                    Add-Diagnostic 'HOOK' "Stage '$($hookStatus.stage)'$(
                        if ($hookStatus.message) { ": $($hookStatus.message)" } else { '.' }
                    )"
                } catch {
                    Add-Diagnostic 'HOOK' "Hook sentinel changed but could not be parsed yet: $(Get-CompactError $_)"
                }
            }
        }
        if (Test-Path -LiteralPath $statusPath -PathType Leaf) {
            ...
            if ($statusItem.LastWriteTimeUtc -gt $lastStatusTimestamp) {
                ...
                try {
                    ...
                    $entries = if ($null -ne $status.history) { @($status.history) } else { @($status) }
                    foreach ($entry in $entries) {
                        $signature = "$($entry.sequence):$($entry.stage):$($entry.message)"
                        if ($reportedStatusStages.Add($signature)) {
                            Add-Diagnostic 'EXPORTER' "Stage '$($entry.stage)'$(
                                if ($entry.message) { ": $($entry.message)" } else { '.' }
                            )"
                        }
                        if ($status.protocol -eq 'jc-scrapmap-export-status-v1' -and
                            $entry.stage -eq 'exported') {
                            $exportReady = $true
                        }
                    }
                } catch {
