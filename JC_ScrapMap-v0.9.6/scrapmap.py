...
def read_saved_vehicles(path: Path) -> dict:
    """Read movable overworld creations containing a recognized engine."""
    uri = f"{path.resolve().as_uri()}?mode=ro"
    connection = sqlite3.connect(uri, uri=True)
    try:
        connection.execute("PRAGMA query_only = ON")
        ...
        for shape_a, shape_b in connection.execute(
            "SELECT childShapeIdA, childShapeIdB FROM Joint"
        ):
        ...
        for body_id, data in connection.execute("SELECT bodyId, data FROM ChildShape"):
            label = ENGINE_UUID_BYTES.get(data[11:27]) if len(data) >= 27 else None
            if label:
                engines_by_body.setdefault(int(body_id), []).append(label)

        visited: set[int] = set()
        vehicles = []
        for engine_body in engines_by_body:
            if engine_body in visited:
                continue
            if not bodies.get(engine_body, {}).get("movable"):
                visited.add(engine_body)
                continue
            component = set()
            pending = [engine_body]
            while pending:
            ...
            if not component_engines or not component_bodies:
                continue
        ...
        return {
            ...
            "vehicles": vehicles,
            "count": len(vehicles),
            "message": f"Found {len(vehicles)} movable engine-powered creations.",
        }
...
def build_state(game_path: Path, user_dirs: list[Path], saves: list[Path], selected: Path) -> dict:
    ...
    state = {
        ...
        "layers": [
            ...
            {
                "id": "vehicles",
                "label": "Vehicles (engine-powered creations)",
