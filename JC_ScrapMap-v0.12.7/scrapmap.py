...
from investigation.inspect_underground_terrain import summarize_world
...
CELL_SIZE_METERS = 64
PLAYER_SNAPSHOT_SETTLE_SECONDS = 0.5
PLAYER_SNAPSHOT_SIDECARS = ("-wal", "-shm", "-journal")
ENGINE_UUIDS = {
...
RESCUE_VEHICLE_UUIDS = {
    "scrapEngine": "c96ab903-f238-4bae-a614-28a758716d00",
    "scrapMetal": "1f7ac0bb-ad45-4246-9817-59bdf7f7ab39",
    "scrapWheel": "59f6951a-a450-42bf-ad03-54567cb70245",
    "scrapSeat": "847daf20-02bf-4170-8699-9ab106acd29a",
    "portableCraftbot": "05900757-a49f-4907-b498-96bfd2b15176",
}
RESCUE_VEHICLE_UUID_BYTES = {
    name: uuid.UUID(identifier).bytes[::-1]
    for name, identifier in RESCUE_VEHICLE_UUIDS.items()
}
RECYCLER_UUIDS = {
    "scrapMetal": "1f7ac0bb-ad45-4246-9817-59bdf7f7ab39",
    "scrapWood": "1fc74a28-addb-451a-878d-c3c605d63811",
    "portableCraftbot": "05900757-a49f-4907-b498-96bfd2b15176",
    "largeChest": "e9efc008-8fae-4391-9ad1-6a62dbab5760",
    "toiletPaper": "e2a0770b-9bb9-4b3b-b9f2-994b534b79f3",
}
RECYCLER_UUID_BYTES = {
    name: uuid.UUID(identifier).bytes[::-1]
    for name, identifier in RECYCLER_UUIDS.items()
}
RECYCLER_RECOVERY_RATE = Fraction(1, 2)
RECYCLER_RECIPE_FILES = (
...
UNDERGROUND_FLOOR_KINDS = {
    1: ("facility", False),
    2: ("authored-mine", True),
    3: ("facility", False),
    4: ("procedural-mine", True),
    5: ("facility", False),
    6: ("procedural-mine", True),
    7: ("facility", False),
    8: ("boss", False),
}
...
WAREHOUSE_STORAGE_CHANNEL = 10
ELEVATOR_STORAGE_CHANNEL = 11
ELEVATOR_SCRIPT_UID = bytes.fromhex("5130C47B0A1953FCA5A1205C19264574")
WAREHOUSE_DESTRUCTION_UID = uuid.UUID(
    "4a825c08-cd87-4a15-9bc5-ffec9fdb4b9d"
).bytes[::-1]
...
class LuaObjectReader:
    ...
    def read_value(self):
        ...
        if value_type == 100:
            ...
            if userdata_type == 10023:
                return {"type": "interactable", "id": self.read_bits(32)}
            if userdata_type == 10027:
                return {"type": "world", "id": self.read_bits(32)}
            if userdata_type == 10036:
                return {"type": "portal", "id": self.read_bits(32)}
            if userdata_type == 10039:
                return {"type": "scriptableObject", "id": self.read_bits(32)}
...
class LuaObjectWriter:
    """Encode the conservative Lua-object subset used by Warehouse storage."""
    ...
    def write_bits(self, value: int, count: int) -> None:
        if value < 0 or value >= 1 << count:
            raise ValueError("Lua-object integer does not fit its encoded width.")
    ...
    def write_value(self, value) -> None:
        ...
        if isinstance(value, float):
            self.write_bits(3, 8)
            self.write_bits(int.from_bytes(struct.pack(">f", value), "big"), 32)
            return
        if isinstance(value, str):
            encoded = value.encode("utf-8")
            self.write_bits(4, 8)
            self.write_bits(len(encoded), 32)
            self.write_bytes(encoded)
            return
        if isinstance(value, int):
            if not -(1 << 31) <= value < (1 << 31):
                raise ValueError("Lua-object integer is outside the signed 32-bit range.")
            self.write_bits(6, 8)
            self.write_bits(value & 0xFFFFFFFF, 32)
            return
        if isinstance(value, dict):
            if value.get("type") == "vec3" and set(value) == {"type", "x", "y", "z"}:
                coordinates = (value["x"], value["y"], value["z"])
                if not all(isinstance(item, (int, float)) for item in coordinates):
                    raise ValueError("Lua vec3 contains an invalid coordinate.")
                self.write_bits(100, 8)
                self.write_bits(10003, 32)
                for coordinate in coordinates:
                    self.write_bits(
                        int.from_bytes(struct.pack(">f", float(coordinate)), "big"),
                        32,
                    )
                return
            userdata_type = self.USERDATA_TYPES.get(value.get("type"))
            if userdata_type is not None and set(value) == {"type", "id"}:
                object_id = value["id"]
                if not isinstance(object_id, int) or not 0 <= object_id <= 0xFFFFFFFF:
                    raise ValueError("Lua userdata has an invalid object identifier.")
                self.write_bits(100, 8)
                self.write_bits(userdata_type, 32)
                self.write_bits(object_id, 32)
                return
            self.write_bits(5, 8)
            integer_keys = sorted(key for key in value if isinstance(key, int))
            is_array = (
                len(integer_keys) == len(value)
                and integer_keys == list(range(1, len(value) + 1))
            )
            self.write_bits(len(value), 32)
            self.write_bits(int(is_array), 1)
            if is_array:
                self.write_bits(1, 32)
                for index in integer_keys:
                    self.write_value(value[index])
            else:
                for key, item in value.items():
                    self.write_value(key)
                    self.write_value(item)
            return
        raise ValueError(f"Unsupported Lua-object value for encoding: {type(value).__name__}.")
...
def compress_lz4_literal_block(data: bytes) -> bytes:
    """Return a valid raw LZ4 block containing one literal-only sequence."""
    length = len(data)
    ...
    while remaining >= 255:
        ...
        remaining -= 255
    if length >= 15:
        output.append(max(remaining, 0))
    output.extend(data)
    return bytes(output)


def encode_lua_object_blob(original: bytes, value) -> bytes:
    """Replace one ScriptData BlobData payload while preserving its record header."""
    if len(original) < 25:
        raise ValueError("ScriptData blob is too short.")
    key_size = int.from_bytes(original[16:18], "big")
    header_size = 16 + 2 + key_size + 2 + 1 + 4
    if header_size > len(original):
        raise ValueError("ScriptData BlobData header is truncated.")
    writer = LuaObjectWriter()
    writer.write_value(value)
    payload = b"LUA\x00\x00\x00\x01" + writer.finish()
    compressed = compress_lz4_literal_block(payload)
    return (
        original[: header_size - 4]
        + len(compressed).to_bytes(4, "big")
        + compressed
    )


def lua_string_key(value: str) -> bytes:
    encoded = value.encode("utf-8")
    return b"LUA\x00\x00\x00\x01\x04" + len(encoded).to_bytes(4, "big") + encoded


def portal_without_opening_b(data: bytes, portal_id: int) -> bytes:
    """Convert one persisted two-opening Portal record into an A-only portal."""
    if len(data) != 126 or data[0] != 9:
        raise ValueError("The Warehouse exterior portal has an unsupported format.")
    if int.from_bytes(data[3:7], "big") != portal_id:
        raise ValueError("The Warehouse exterior portal identifier is inconsistent.")
    if data[18] != 0xFF or data[27:29] == b"\xff\xff":
        raise ValueError("The Warehouse exterior portal has no removable B opening.")
    result = (
        data[:18]
        + b"\xff"
        + b"\x00" * 8
    ...
    if len(result) != 96:
        raise ValueError("The one-sided Warehouse exterior portal could not be encoded.")
    return result
...
def read_generated_json(path: Path):
    """Read a game JSON file after removing its generated-file comments."""
...
@lru_cache(maxsize=4)
def cached_recycler_catalog(game_path_text: str) -> dict:
    """Load installed item recipes, names, and stack limits for recycling."""
    game_path = Path(game_path_text)
    recipe_root = game_path / "Survival" / "CraftingRecipes"
    recipe_paths = sorted((recipe_root / "craftbot").glob("*.json"))
    recipe_paths = [path for path in recipe_paths if path.name != "craftbot.json"]
    recipe_paths.extend(recipe_root / name for name in RECYCLER_RECIPE_FILES)
    recipe_variants: dict[str, set[tuple[int, tuple[tuple[str, int], ...]]]] = {}
    catalog_hash = hashlib.sha256()
    for path in recipe_paths:
        if not path.is_file():
            continue
        raw = path.read_bytes()
        catalog_hash.update(path.name.encode("utf-8"))
        catalog_hash.update(raw)
        document = read_generated_json(path)
        ...
        for recipe in document:
            ...
            output_quantity = recipe.get("quantity")
            ingredients = recipe.get("ingredientList")
            if (
            ...
            for ingredient in ingredients:
                ingredient_id = ingredient.get("itemId") if isinstance(ingredient, dict) else None
                quantity = ingredient.get("quantity") if isinstance(ingredient, dict) else None
                if (
                    not isinstance(ingredient_id, str)
                    or not isinstance(quantity, int)
                    or quantity <= 0
                ):
                    valid = False
                    break
                ingredient_counts[ingredient_id] += quantity
            if valid and ingredient_counts:
                signature = (
                    output_quantity,
                    tuple(sorted(ingredient_counts.items())),
                )
                recipe_variants.setdefault(item_id, set()).add(signature)

    recipes = {
        item_id: next(iter(variants))
        for item_id, variants in recipe_variants.items()
        if len(variants) == 1
    }
    ambiguous = {
        item_id for item_id, variants in recipe_variants.items() if len(variants) > 1
    }

    names: dict[str, str] = {}
    description_paths = (
        game_path / "Data" / "Gui" / "Language" / "English" / "InventoryItemDescriptions.json",
        game_path / "Survival" / "Gui" / "Language" / "English" / "inventoryDescriptions.json",
    )
    for path in description_paths:
        if not path.is_file():
            continue
        catalog_hash.update(path.read_bytes())
        document = read_generated_json(path)
        if not isinstance(document, dict):
            continue
        for item_id, description in document.items():
            if isinstance(description, dict) and isinstance(description.get("title"), str):
                names[item_id] = description["title"]

    stack_sizes: dict[str, int] = {}
    shape_roots = (
        game_path / "Data" / "Objects" / "Database" / "ShapeSets",
        game_path / "Survival" / "Objects" / "Database" / "ShapeSets",
    )
    for root in shape_roots:
        for path in sorted(root.glob("*.shapeset")):
            document = read_generated_json(path)
            for item in walk_json_objects(document):
                item_id = item.get("uuid")
                stack_size = item.get("stackSize")
                if (
                    isinstance(item_id, str)
                    and isinstance(stack_size, int)
                    and 0 < stack_size <= 65535
                ):
                    stack_sizes[item_id] = stack_size
    ...
    return {
        ...
        "hash": catalog_hash.hexdigest(),
    }


def load_recycler_catalog(game_path: Path) -> dict:
    return cached_recycler_catalog(str(game_path.resolve()))
...
def save_snapshot_signature(path: Path) -> dict[str, tuple[int, int] | None]:
    """Return non-invasive size/time metadata for a save and its journals."""
    signature = {}
    for suffix in ("", *PLAYER_SNAPSHOT_SIDECARS):
        ...
        try:
            ...
            signature[suffix] = (stat.st_size, stat.st_mtime_ns)
    return signature


def read_player_position_from_raw_snapshot(
    path: Path,
    *,
    settle_seconds: float = PLAYER_SNAPSHOT_SETTLE_SECONDS,
) -> dict:
    """Copy a quiet save without SQLite access, then read PlayerData locally."""
    before = save_snapshot_signature(path)
    if before[""] is None:
        raise RuntimeError("The selected save is no longer available.")
    if settle_seconds > 0:
        time.sleep(settle_seconds)
    stable = save_snapshot_signature(path)
    if before != stable or stable["-journal"] is not None:
        raise RuntimeError("Save is busy; wait a moment and try again.")

    try:
        with tempfile.TemporaryDirectory(prefix="jc-scrapmap-player-") as temporary:
            snapshot_dir = Path(temporary)
            snapshot_path = snapshot_dir / path.name
            shutil.copy2(path, snapshot_path)
            if stable["-wal"] is not None:
                shutil.copy2(Path(f"{path}-wal"), Path(f"{snapshot_path}-wal"))
            if save_snapshot_signature(path) != stable:
                raise RuntimeError("Save changed while it was copied; try again.")

            position = read_saved_player_position(snapshot_path)
            persisted_times = [
                metadata[1]
                for metadata in stable.values()
                if metadata is not None
            ]
            position["savedUtc"] = datetime.fromtimestamp(
                max(persisted_times) / 1_000_000_000,
                tz=timezone.utc,
            ).isoformat()
            return position
    except RuntimeError:
        ...
        raise RuntimeError(
            "The temporary player-position snapshot could not be validated; try again."
        ) from error
...
def build_excavation_island_map(
    save_path: Path,
    game_path: Path,
    saved_beacons: dict | None = None,
) -> dict:
    """Build the fixed Excavation Island surface map from installed game data."""
    generator_path = (
        game_path
        / "Survival"
        / "Scripts"
    ...
    try:
        ...
        match = re.search(
            r"ExcavationIsland\s*=\s*\{\s*x\s*=\s*(-?\d+)\s*,\s*"
            r"y\s*=\s*(-?\d+)\s*,\s*worldFile\s*=\s*\"([^\"]+)\"\s*,\s*"
            r"rotation\s*=\s*(\d+)",
            generator_text,
        )
        if not match:
            raise ValueError("Excavation Island definition was not found.")
        inject_x, inject_y = int(match.group(1)), int(match.group(2))
        world_reference = match.group(3)
        world_rotation = int(match.group(4)) % 4
        if world_rotation != 0:
            raise ValueError("Rotated Excavation Island worlds are not supported yet.")

        world_relative = world_reference.replace("$SURVIVAL_DATA/", "")
        world_path = game_path / "Survival" / Path(world_relative)
        world_document = json.loads(world_path.read_text(encoding="utf-8-sig"))
        world_cells = [
            cell for cell in world_document.get("cellData", [])
            if isinstance(cell, dict)
        ]
        world_corners = [
            corner for corner in world_document.get("cornerData", [])
            if isinstance(corner, dict)
        ]
        if not world_cells or not world_corners:
            raise ValueError("Excavation Island authored terrain is empty.")

        min_cell_x = min(int(cell["x"]) for cell in world_cells)
        max_cell_x = max(int(cell["x"]) for cell in world_cells)
        min_cell_y = min(int(cell["y"]) for cell in world_cells)
        max_cell_y = max(int(cell["y"]) for cell in world_cells)
        translate_x = inject_x + math.ceil((max_cell_x - min_cell_x) / 2)
        translate_y = inject_y + math.ceil((max_cell_y - min_cell_y) / 2)

        corner_types = {
            (int(corner["x"]), int(corner["y"])): int(corner["type"])
        ...
        terrain_cells = []
        for local_y in range(min_cell_y, max_cell_y + 1):
            for local_x in range(min_cell_x, max_cell_x + 1):
                types = [
                    corner_types.get((local_x, local_y), 8),
                    corner_types.get((local_x + 1, local_y), 8),
                    corner_types.get((local_x, local_y + 1), 8),
                    corner_types.get((local_x + 1, local_y + 1), 8),
                ]
                counts = Counter(types)
                terrain_type = max(
                    counts,
                    key=lambda value: (counts[value], value != TERRAIN_LAKE),
                )
                terrain_cells.append(
                    {
                        "cellX": translate_x + local_x,
                        "cellY": translate_y + local_y,
                        "terrain": terrain_names.get(terrain_type, "unknown"),
                    }
                )

        tile_cache: dict[str, tuple[Path, dict]] = {}
        tile_cells = []
        placements: dict[tuple[str, int, int, int], dict] = {}
        for cell in world_cells:
            virtual_path = cell.get("path")
            if not isinstance(virtual_path, str) or not virtual_path:
                continue
            if virtual_path not in tile_cache:
                relative = virtual_path.replace("$SURVIVAL_DATA/", "")
                tileson_path = game_path / "Survival" / Path(relative).with_suffix(".tileson")
                tile_cache[virtual_path] = (
                    tileson_path,
                    json.loads(tileson_path.read_text(encoding="utf-8-sig")),
                )
            tileson_path, tile_document = tile_cache[virtual_path]
            info = tile_document.get("info", {})
            size_x = int(info.get("cellsX", 1))
            size_y = int(info.get("cellsY", 1))
            if size_x != size_y:
                raise ValueError(f"Non-square excavation tile: {tileson_path.name}")
            size = size_x
            rotation = int(cell.get("rotation", 0)) % 4
            offset_x = int(cell.get("offsetX", 0))
            offset_y = int(cell.get("offsetY", 0))
            if rotation == 0:
                within_x, within_y = offset_x, offset_y
            elif rotation == 1:
                within_x, within_y = size - 1 - offset_y, offset_x
            elif rotation == 2:
                within_x, within_y = size - 1 - offset_x, size - 1 - offset_y
            else:
                within_x, within_y = offset_y, size - 1 - offset_x
            origin_x = int(cell["x"]) - within_x
            origin_y = int(cell["y"]) - within_y
            key = (virtual_path, origin_x, origin_y, rotation)
            placements.setdefault(
                key,
                {
                    "virtualPath": virtual_path,
                    "developerTile": tileson_path.stem,
                    "originCellX": translate_x + origin_x,
                    "originCellY": translate_y + origin_y,
                    "sizeCells": size,
                    "rotation": rotation,
            ...
            tile_cells.append(
                {
                    "cellX": translate_x + int(cell["x"]),
                    "cellY": translate_y + int(cell["y"]),
                    "tile": tileson_path.stem,
                }
            )

        def rotate_point(x: float, y: float, size_meters: float, rotation: int):
            if rotation == 1:
                return size_meters - y, x
            if rotation == 2:
                return size_meters - x, size_meters - y
            if rotation == 3:
                return y, size_meters - x
            return x, y

        def prefab_label(path_value: str) -> str | None:
            name = Path(path_value).stem.casefold()
            labels = (
                ("surfaceelevator", "Excavation elevator"),
                ("massivedigger", "Massive digger"),
                ("massiveplatform", "Massive platform"),
                ("controlbooth", "Control booth"),
                ("chemicalplant", "Chemical plant"),
                ("oilspill_robot", "Oil-spill robot"),
                ("diggerbot", "Digger bot"),
                ("outhouse", "Outhouse"),
            )
            return next((label for needle, label in labels if needle in name), None)
        ...
        for placement in placements.values():
            size_meters = placement["sizeCells"] * CELL_SIZE_METERS
            origin_world_x = placement["originCellX"] * CELL_SIZE_METERS
            origin_world_y = placement["originCellY"] * CELL_SIZE_METERS
            entities = placement["document"].get("entities", {})
            for category in point_categories:
                for entity in entities.get(category, []):
                    ...
                    if category == "nodes":
                        if "EPICLOOTCRATE" in tags:
                            kind = "epic-loot"
                        elif "LOOTCRATE" in tags:
                            kind = "loot"
                        elif "SPAWNABLE_UNIT" in tags:
                            kind = "enemy-spawn"
                        elif "POTATO_SPAWN" in tags:
                            kind = "potato-spawn"
                        elif "QUESTMARKER" in tags:
                            kind = "quest"
                        else:
                            kind = "node"
                    path_value = entity.get("path", "")
                    label = prefab_label(path_value) if category == "prefabs" else None
                    if category == "prefabs" and "ruins_kit" in path_value.casefold():
                        kind = "ruin"
                    point = {
                        "category": category,
                        "kind": kind,
                        "x": round(origin_world_x + rotated_x, 3),
                        "y": round(origin_world_y + rotated_y, 3),
                        "z": round(float(position[2]), 3) if len(position) > 2 else None,
                    }
                    points.append(point)
                    if label and (
                        abs(float(position[0])) > 1 or abs(float(position[1])) > 1
                    ):
                        label_entry = {
                            "title": label,
                            "x": point["x"],
                        ...
                        if label_entry not in labels:
                            labels.append(label_entry)

        bridge = next(
            (
                placement for placement in placements.values()
                if "KinematicBridge" in placement["developerTile"]
            ),
            None,
        )
        if bridge:
            labels.append(
                {
                    "title": "Island access bridge",
                    "x": (bridge["originCellX"] + bridge["sizeCells"] / 2) * CELL_SIZE_METERS,
                    "y": (bridge["originCellY"] + bridge["sizeCells"] / 2) * CELL_SIZE_METERS,
                }
            )
        ...
        try:
            connection.execute("PRAGMA query_only = ON")
            island_min_x = translate_x + min_cell_x
            island_max_x = translate_x + max_cell_x
            island_min_y = translate_y + min_cell_y
            island_max_y = translate_y + max_cell_y
            row = connection.execute(
                "SELECT worldIdA, xA, yA, worldIdB, xB, yB FROM Portal "
                "WHERE (worldIdA = 1 AND xA BETWEEN ? AND ? AND yA BETWEEN ? AND ?) "
                "OR (worldIdB = 1 AND xB BETWEEN ? AND ? AND yB BETWEEN ? AND ?) "
                "LIMIT 1",
                (
                    island_min_x,
                    island_max_x,
            ...
            if row:
                world_a, x_a, y_a, world_b, x_b, y_b = (int(value) for value in row)
                cell_x, cell_y, destination = (
                    (x_a, y_a, world_b) if world_a == 1 else (x_b, y_b, world_a)
                )
                elevator = {
                    "cellX": cell_x,
                    "cellY": cell_y,
                    ...
                    "positionAccuracy": "Exact saved portal cell",
                }
        finally:
            connection.close()
        ...
        placement_output = []
        for placement in placements.values():
            placement_output.append(
                {
                    key: value for key, value in placement.items()
                    if key != "document"
                }
            )
        point_counts = Counter(point["kind"] for point in points)
        all_min_x = min(
            [translate_x + min_cell_x]
            + [placement["originCellX"] for placement in placements.values()]
        )
        all_max_x = max(
            [translate_x + max_cell_x + 1]
            + [
                placement["originCellX"] + placement["sizeCells"]
                for placement in placements.values()
            ]
        )
        all_min_y = min(
            [translate_y + min_cell_y]
            + [placement["originCellY"] for placement in placements.values()]
        )
        all_max_y = max(
            [translate_y + max_cell_y + 1]
            + [
                placement["originCellY"] + placement["sizeCells"]
                for placement in placements.values()
            ]
        )
        bounds = {
            "xMin": all_min_x * CELL_SIZE_METERS,
            "xMax": all_max_x * CELL_SIZE_METERS,
            "yMin": all_min_y * CELL_SIZE_METERS,
            "yMax": all_max_y * CELL_SIZE_METERS,
        }
        island_beacons = [
            dict(beacon)
            for beacon in (saved_beacons or {}).get("beacons", [])
            if beacon.get("worldId") == 1
            and bounds["xMin"] <= beacon.get("x", math.inf) <= bounds["xMax"]
            and bounds["yMin"] <= beacon.get("y", math.inf) <= bounds["yMax"]
        ]
        document = {
            "status": "available",
            ...
            "placements": placement_output,
            "points": points,
            "labels": labels,
            "elevator": elevator,
            "beacons": island_beacons,
            "summary": {
                "terrainCells": len(terrain_cells),
                "authoredTileCells": len(tile_cells),
                "schematicPoints": len(points),
                "ruinPoints": point_counts["ruin"],
                "lootPoints": point_counts["loot"],
                "epicLootPoints": point_counts["epic-loot"],
                "enemySpawnPoints": point_counts["enemy-spawn"],
                "beaconCount": len(island_beacons),
            },
        }
        filename = "excavation-island.json"
        ...
        return {
            ...
            "message": "Fixed authored Excavation Island surface map is available.",
        }
    except (
        OSError,
        ValueError,
        ...
        LOGGER.warning("Excavation Island map unavailable category=%s.", type(error).__name__)
        return {
            "status": "unavailable",
            "message": f"Excavation Island map is unavailable: {error}",
        }
...
def inspect_generator_confirmed_map(game_path: Path) -> dict:
    ...
    for index, match in enumerate(poi_pattern.finditer(fixed_section), start=1):
        poi_center_x = int(match.group(1))
        poi_center_y = int(match.group(2))
        ...
        cell_x = poi_center_x - size // 2
        cell_y = poi_center_y - size // 2
        features.append(
            {
    ...
    if excavation_origin:
        ...
        cell_x, cell_y, size = island_x, island_y, 32
        entrance_features.append(
            {
                ...
                "undergroundMapStatus": "available",
                "undergroundMessage": (
                    "The optional Excavation Island surface map is available."
                ),
                "excavationIslandMap": True,
            }
        )
...
def warehouse_restorations_path(save_identity: str) -> Path:
    return MAPPER_DATA_DIR / save_identity / "warehouse-restorations.json"


def load_warehouse_restorations(save_identity: str) -> dict:
    path = warehouse_restorations_path(save_identity)
    if not path.is_file():
        return {}
    try:
        document = json.loads(path.read_text(encoding="utf-8"))
        ...
        raise ValueError(
            "The warehouse restoration registry cannot be read safely."
        ) from error
    entries = document.get("warehouses", {})
    if not isinstance(entries, dict):
        raise ValueError("The warehouse restoration registry has an invalid format.")
    return entries


def write_warehouse_restorations(save_identity: str, entries: dict) -> Path:
    path = warehouse_restorations_path(save_identity)
    path.parent.mkdir(parents=True, exist_ok=True)
    document = {
        "schemaVersion": 1,
...
def warehouse_feature_id(cell_x: int, cell_y: int) -> str:
    return f"regular-warehouse-{cell_x}-{cell_y}"


def read_saved_warehouses(path: Path, save_identity: str) -> dict:
    """Read warehouse lifecycle state without changing the selected save."""
    try:
        registry = load_warehouse_restorations(save_identity)
    except ValueError:
        return {"status": "unavailable", "warehouses": {}}
    uri = f"{path.resolve().as_uri()}?mode=ro"
    connection = sqlite3.connect(uri, uri=True)
    try:
        connection.execute("PRAGMA query_only = ON")
        row = connection.execute(
            "SELECT data FROM ScriptData WHERE uid = ? AND key = ? LIMIT 1",
            (GLOBAL_STORAGE_UID, global_storage_key(WAREHOUSE_STORAGE_CHANNEL)),
        ).fetchone()
        if row is None:
            return {"status": "unavailable", "warehouses": {}}
        decoded = decode_lua_object(bytes(row[0]))
        ...
        for entry in decoded.values():
            if not isinstance(entry, dict):
                continue
            zero_cell = entry.get("zeroCell")
            if not isinstance(zero_cell, dict):
                continue
            cell_x, cell_y = zero_cell.get("x"), zero_cell.get("y")
            if not isinstance(cell_x, int) or not isinstance(cell_y, int):
                continue
            if entry.get("isQuestWarehouse") is not False:
                continue
            feature_id = warehouse_feature_id(cell_x, cell_y)
            restored = registry.get(feature_id)
            destroyed = entry.get("destroyed") is True
            warehouses[feature_id] = {
                "status": (
                    "restored-once"
                    if isinstance(restored, dict)
                    else "destroyed" if destroyed else "intact"
                ),
                "destroyed": destroyed,
                "eligible": destroyed and not isinstance(restored, dict),
                "restoredOnce": isinstance(restored, dict),
            }
        return {"status": "available", "warehouses": warehouses}
...
def revive_saved_warehouse(path: Path, save_identity: str, feature_id: str) -> dict:
    """Restore one regular warehouse and orphan its destroyed interior worlds."""
    match = re.fullmatch(r"regular-warehouse-(-?\d+)-(-?\d+)", feature_id)
    if match is None:
        raise ValueError("Invalid regular warehouse identifier.")
    cell_x, cell_y = (int(value) for value in match.groups())
    restorations = load_warehouse_restorations(save_identity)
    if feature_id in restorations:
        raise ValueError("This warehouse has already used its one restoration.")

    # Confirm that the one-use registry is writable before touching the save.
    write_warehouse_restorations(save_identity, restorations)
    connection = sqlite3.connect(path, timeout=5)
    try:
        connection.execute("BEGIN IMMEDIATE")
        registry_key = global_storage_key(WAREHOUSE_STORAGE_CHANNEL)
        registry_row = connection.execute(
            "SELECT data FROM ScriptData WHERE uid = ? AND key = ? LIMIT 1",
            (GLOBAL_STORAGE_UID, registry_key),
        ).fetchone()
        if registry_row is None:
            raise ValueError("The save has no warehouse registry.")
        original_registry_blob = bytes(registry_row[0])
        warehouse_registry = decode_lua_object(original_registry_blob)
        candidates = [
            entry
            for entry in warehouse_registry.values()
            if isinstance(entry, dict)
            and isinstance(entry.get("zeroCell"), dict)
            and entry["zeroCell"].get("x") == cell_x
            and entry["zeroCell"].get("y") == cell_y
        ] if isinstance(warehouse_registry, dict) else []
        if len(candidates) != 1:
            raise ValueError("The selected warehouse was not found in this save.")
        warehouse = candidates[0]
        if warehouse.get("isQuestWarehouse") is not False:
            raise ValueError("Quest warehouses cannot be restored.")
        if warehouse.get("destroyed") is not True:
            raise ValueError("The selected warehouse is not destroyed.")
        if warehouse.get("tileStorageKey") != f"ts_1:({cell_x},{cell_y})":
            raise ValueError("The warehouse registry does not match the selected map POI.")

        old_world_ids = {
            item["id"]
            for item in warehouse.get("worlds", {}).values()
            if isinstance(item, dict)
            and item.get("type") == "world"
            and isinstance(item.get("id"), int)
        } if isinstance(warehouse.get("worlds"), dict) else set()
        if not old_world_ids:
            raise ValueError("The destroyed warehouse has no interior worlds to replace.")
        placeholders = ",".join("?" for _ in old_world_ids)
        portal_rows = list(connection.execute(
            f"SELECT id, worldIdA, xA, yA, worldIdB, xB, yB, data FROM Portal "
            f"WHERE worldIdA IN ({placeholders}) OR worldIdB IN ({placeholders})",
            (*old_world_ids, *old_world_ids),
        ))
        portal_ids = {int(row[0]) for row in portal_rows}
        if not portal_ids:
            raise ValueError("The warehouse interior portals could not be identified.")

        tile_range = warehouse.get("tileRange")
        if not isinstance(tile_range, dict) or not all(
            isinstance(tile_range.get(key), int)
            for key in ("xMin", "xMax", "yMin", "yMax")
        ):
            raise ValueError("The warehouse exterior range is missing.")
        elevator_rows = []
        for uid, key, data in connection.execute(
            "SELECT uid, key, data FROM ScriptData WHERE uid = ? AND worldId = 1",
            (ELEVATOR_SCRIPT_UID,),
        ):
            try:
                elevator = decode_lua_object(bytes(data))
            except ValueError:
                continue
            portal = elevator.get("portal") if isinstance(elevator, dict) else None
            if (
                isinstance(elevator, dict)
                and elevator.get("name") in {"ELEVATOR_ENTRANCE", "ELEVATOR_EXIT"}
                and isinstance(elevator.get("x"), int)
                and isinstance(elevator.get("y"), int)
                and tile_range.get("xMin") <= elevator["x"] <= tile_range.get("xMax")
                and tile_range.get("yMin") <= elevator["y"] <= tile_range.get("yMax")
                and isinstance(portal, dict)
                and portal.get("id") in portal_ids
            ):
                encoded_key = bytes(key)
                if len(encoded_key) != 4:
                    raise ValueError("A Warehouse elevator has an invalid storage key.")
                elevator_rows.append({
                    "uid": bytes(uid),
                    "key": encoded_key,
                    "blob": bytes(data),
                    "saved": elevator,
                    "controllerId": int.from_bytes(encoded_key, "little"),
                    "portalId": int(portal["id"]),
                })
        elevators_by_name = {
            row["saved"]["name"]: row for row in elevator_rows
        }
        if len(elevator_rows) != 2 or set(elevators_by_name) != {
            "ELEVATOR_ENTRANCE", "ELEVATOR_EXIT"
        }:
            raise ValueError("The warehouse exterior elevators could not be identified.")
        entrance = elevators_by_name["ELEVATOR_ENTRANCE"]
        exit_elevator = elevators_by_name["ELEVATOR_EXIT"]
        entrance_portal = next(
            (row for row in portal_rows if int(row[0]) == entrance["portalId"]),
            None,
        )
        if (
            entrance_portal is None
            or int(entrance_portal[1]) != 1
            or int(entrance_portal[4]) not in old_world_ids
        ):
            raise ValueError("The Warehouse entrance portal is not safely reusable.")
        exterior_portals = [
            row for row in portal_rows
            if int(row[1]) == 1 and int(row[4]) in old_world_ids
        ]
        if entrance_portal not in exterior_portals:
            raise ValueError("The Warehouse entrance portal is not exterior-origin.")
        one_sided_exterior_portals = {
            int(row[0]): portal_without_opening_b(bytes(row[7]), int(row[0]))
            for row in exterior_portals
        }
        emergency_portal_ids = sorted(
            portal_id for portal_id in one_sided_exterior_portals
            if portal_id != entrance["portalId"]
        )

        elevator_registry_key = global_storage_key(ELEVATOR_STORAGE_CHANNEL)
        elevator_registry_row = connection.execute(
            "SELECT data FROM ScriptData WHERE uid = ? AND key = ? LIMIT 1",
            (GLOBAL_STORAGE_UID, elevator_registry_key),
        ).fetchone()
        if elevator_registry_row is None:
            raise ValueError("The save has no elevator registry.")
        original_elevator_blob = bytes(elevator_registry_row[0])
        elevator_registry = decode_lua_object(original_elevator_blob)
        if not isinstance(elevator_registry, dict):
            raise ValueError("The elevator registry has an unsupported format.")
        cleaned_elevators = {
            key: item for key, item in elevator_registry.items()
            if not (
                key in portal_ids
                or (
                    isinstance(item, dict)
                    and isinstance(item.get("portal"), dict)
                    and item["portal"].get("id") in portal_ids
                )
            )
        }
        cleaned_elevators[entrance["portalId"]] = {
            "destination": "b",
            "a": {"type": "interactable", "id": entrance["controllerId"]},
            "portal": {"type": "portal", "id": entrance["portalId"]},
            "ticksToDestination": 0,
            "reverse": False,
        }
        exit_saved = dict(exit_elevator["saved"])
        exit_saved.pop("portal", None)

        exterior_key = lua_string_key(f"ts_1:({cell_x},{cell_y})sync")
        exterior_rows = list(connection.execute(
            "SELECT uid, data FROM ScriptData WHERE key = ?", (exterior_key,)
        ))
        if len(exterior_rows) != 1:
            raise ValueError("The warehouse exterior state could not be identified safely.")
        exterior_uid, exterior_data = exterior_rows[0]
        original_exterior_blob = bytes(exterior_data)
        exterior = decode_lua_object(original_exterior_blob)
        flags = exterior.get("tilestorageTagFlags") if isinstance(exterior, dict) else None
        if not isinstance(flags, dict):
            raise ValueError("The warehouse exterior destruction flags are missing.")
        removed_flags = [
            key for key in tuple(flags)
            if key == "destroyed" or re.fullmatch(r"explosion\d+", str(key))
        ]
        if "destroyed" not in removed_flags:
            raise ValueError("The warehouse exterior is not marked as destroyed.")
        for key in removed_flags:
            flags.pop(key, None)

        countdown = warehouse.get("countdownEvent")
        countdown_id = (
            countdown.get("id")
            if isinstance(countdown, dict)
            and countdown.get("type") == "scriptableObject"
            and isinstance(countdown.get("id"), int)
            else None
        )
        for key in (
            "destroyed", "consoleDestroyed", "countdownEvent",
            "worldRestrictionsLifted", "worldAlertLightingOn",
        ):
        ...
        updates = (
            (
                GLOBAL_STORAGE_UID,
                registry_key,
                original_registry_blob,
                encode_lua_object_blob(original_registry_blob, warehouse_registry),
            ),
            (
                GLOBAL_STORAGE_UID,
                elevator_registry_key,
                original_elevator_blob,
                encode_lua_object_blob(original_elevator_blob, cleaned_elevators),
            ),
            (
                bytes(exterior_uid),
                exterior_key,
                original_exterior_blob,
                encode_lua_object_blob(original_exterior_blob, exterior),
            ),
            (
                exit_elevator["uid"],
                exit_elevator["key"],
                exit_elevator["blob"],
                encode_lua_object_blob(exit_elevator["blob"], exit_saved),
            ),
        )
        for uid, key, original, replacement in updates:
            cursor = connection.execute(
                "UPDATE ScriptData SET data = ? WHERE uid = ? AND key = ? AND data = ?",
                (replacement, uid, key, original),
            )
            if cursor.rowcount != 1:
                raise RuntimeError("The save changed during warehouse restoration.")
        for exterior_portal in exterior_portals:
            exterior_portal_id = int(exterior_portal[0])
            portal_cursor = connection.execute(
                "UPDATE Portal SET worldIdB = 65535, xB = 0, yB = 0, data = ? "
                "WHERE id = ? AND worldIdA = ? AND xA = ? AND yA = ? "
                "AND worldIdB = ? AND xB = ? AND yB = ? AND data = ?",
                (
                    one_sided_exterior_portals[exterior_portal_id],
                    *exterior_portal,
                ),
            )
            if portal_cursor.rowcount != 1:
                raise RuntimeError(
                    "A Warehouse exterior portal changed during revival."
                )
        preserved_portal_ids = set(one_sided_exterior_portals)
        discarded_portal_ids = portal_ids - preserved_portal_ids
        if discarded_portal_ids:
            connection.execute(
                f"DELETE FROM Portal WHERE id IN "
                f"({','.join('?' for _ in discarded_portal_ids)})",
                tuple(discarded_portal_ids),
            )
        if countdown_id is not None:
            countdown_key = countdown_id.to_bytes(4, "little")
            connection.execute("DELETE FROM ScriptableObject WHERE id = ?", (countdown_id,))
            connection.execute(
                "DELETE FROM ScriptData WHERE uid = ? AND key = ?",
                (WAREHOUSE_DESTRUCTION_UID, countdown_key),
            )
            connection.execute(
                "DELETE FROM ScriptDataUuid WHERE uid = ? AND key = ?",
                (WAREHOUSE_DESTRUCTION_UID, countdown_key),
            )
        integrity = connection.execute("PRAGMA integrity_check").fetchone()
        if integrity is None or integrity[0] != "ok":
            raise ValueError("The edited save did not pass SQLite integrity checking.")
        connection.commit()
    except Exception:
        connection.rollback()
        raise
    ...
    restorations[feature_id] = {
        "featureId": feature_id,
        "warehouseIndex": warehouse.get("index"),
        "tileStorageKey": warehouse.get("tileStorageKey"),
        "restoredUtc": restored_utc,
        "orphanedWorldIds": sorted(old_world_ids),
        "elevatorStrategy": "exterior-portals-one-sided-v3",
        "entrancePortalId": entrance["portalId"],
        "emergencyPortalIds": emergency_portal_ids,
    }
    write_warehouse_restorations(save_identity, restorations)
    LOGGER.info(
        "Warehouse restoration result=ok warehouseIndex=%s worlds=%s portals=%s.",
        warehouse.get("index"), len(old_world_ids), len(portal_ids),
    )
    return {
        "featureId": feature_id,
        "restoredUtc": restored_utc,
        "orphanedWorldCount": len(old_world_ids),
    }


def repair_revived_warehouse_elevator(
    path: Path, source_path: Path, save_identity: str, feature_id: str
) -> dict:
    """Repair the entrance records removed by the original 0.12.7 test build."""
    match = re.fullmatch(r"regular-warehouse-(-?\d+)-(-?\d+)", feature_id)
    if match is None:
        raise ValueError("Invalid regular warehouse identifier.")
    cell_x, cell_y = (int(value) for value in match.groups())
    restorations = load_warehouse_restorations(save_identity)
    restoration = restorations.get(feature_id)
    if not isinstance(restoration, dict):
        raise ValueError("This Warehouse has no recorded revival to repair.")
    if restoration.get("elevatorStrategy") == "one-sided-entrance-portal-v2":
        raise ValueError("This Warehouse already uses the corrected elevator strategy.")
    ...
    try:
        ...
        source_seed = source.execute("SELECT seed FROM Game").fetchone()
        target_seed = connection.execute("SELECT seed FROM Game").fetchone()
        if source_seed is None or target_seed is None or source_seed != target_seed:
            raise ValueError("The elevator repair source belongs to another world seed.")

        registry_key = global_storage_key(WAREHOUSE_STORAGE_CHANNEL)

        def load_registry(database: sqlite3.Connection) -> tuple[bytes, dict]:
            row = database.execute(
                "SELECT data FROM ScriptData WHERE uid = ? AND key = ? LIMIT 1",
                (GLOBAL_STORAGE_UID, registry_key),
            ).fetchone()
            if row is None:
                raise ValueError("A Warehouse registry is missing.")
            blob = bytes(row[0])
            decoded = decode_lua_object(blob)
            if not isinstance(decoded, dict):
                raise ValueError("A Warehouse registry has an unsupported format.")
            return blob, decoded

        _, target_registry = load_registry(connection)
        _, source_registry = load_registry(source)

        def exact_warehouse(registry: dict) -> dict:
            matches = [
                entry for entry in registry.values()
                if isinstance(entry, dict)
                and entry.get("zeroCell") == {"x": cell_x, "y": cell_y}
            ]
            if len(matches) != 1:
                raise ValueError("The Warehouse repair target could not be matched.")
            return matches[0]

        target_warehouse = exact_warehouse(target_registry)
        source_warehouse = exact_warehouse(source_registry)
        if (
            target_warehouse.get("isQuestWarehouse") is not False
            or source_warehouse.get("isQuestWarehouse") is not False
        ):
            raise ValueError("Quest Warehouses cannot be repaired by this operation.")
        if target_warehouse.get("destroyed") is True or target_warehouse.get("worlds") != {}:
            raise ValueError("The selected save is not in the expected failed-revival state.")
        if source_warehouse.get("destroyed") is not True:
            raise ValueError("The repair source does not contain the destroyed Warehouse.")
        source_world_ids = {
            item["id"] for item in source_warehouse.get("worlds", {}).values()
            if isinstance(item, dict)
            and item.get("type") == "world"
            and isinstance(item.get("id"), int)
        } if isinstance(source_warehouse.get("worlds"), dict) else set()
        recorded_world_ids = set(restoration.get("orphanedWorldIds", []))
        if not source_world_ids or source_world_ids != recorded_world_ids:
            raise ValueError("The repair source does not match the recorded old interiors.")

        tile_range = source_warehouse.get("tileRange")
        if not isinstance(tile_range, dict) or not all(
            isinstance(tile_range.get(key), int)
            for key in ("xMin", "xMax", "yMin", "yMax")
        ):
            raise ValueError("The repair source has no valid Warehouse tile range.")
        placeholders = ",".join("?" for _ in source_world_ids)
        portal_rows = list(source.execute(
            f"SELECT id, worldIdA, xA, yA, worldIdB, xB, yB, data FROM Portal "
            f"WHERE worldIdA IN ({placeholders}) OR worldIdB IN ({placeholders})",
            (*source_world_ids, *source_world_ids),
        ))
        portal_ids = {int(row[0]) for row in portal_rows}
        source_elevators = []
        for uid, key, world_id, flags, data in source.execute(
            "SELECT uid, key, worldId, flags, data FROM ScriptData "
            "WHERE uid = ? AND worldId = 1",
            (ELEVATOR_SCRIPT_UID,),
        ):
            try:
                saved = decode_lua_object(bytes(data))
            except ValueError:
                continue
            portal = saved.get("portal") if isinstance(saved, dict) else None
            if (
                isinstance(saved, dict)
                and saved.get("name") in {"ELEVATOR_ENTRANCE", "ELEVATOR_EXIT"}
                and tile_range["xMin"] <= saved.get("x", 1 << 30) <= tile_range["xMax"]
                and tile_range["yMin"] <= saved.get("y", 1 << 30) <= tile_range["yMax"]
                and isinstance(portal, dict)
                and portal.get("id") in portal_ids
            ):
                encoded_key = bytes(key)
                if len(encoded_key) != 4:
                    raise ValueError("A repair-source elevator has an invalid key.")
                source_elevators.append({
                    "uid": bytes(uid), "key": encoded_key,
                    "worldId": int(world_id), "flags": int(flags),
                    "blob": bytes(data), "saved": saved,
                    "controllerId": int.from_bytes(encoded_key, "little"),
                    "portalId": int(portal["id"]),
                })
        by_name = {row["saved"]["name"]: row for row in source_elevators}
        if len(source_elevators) != 2 or set(by_name) != {
            "ELEVATOR_ENTRANCE", "ELEVATOR_EXIT"
        }:
            raise ValueError("The repair-source exterior elevators are incomplete.")
        entrance = by_name["ELEVATOR_ENTRANCE"]
        exit_elevator = by_name["ELEVATOR_EXIT"]
        entrance_portal = next(
            (row for row in portal_rows if int(row[0]) == entrance["portalId"]),
            None,
        )
        if (
            entrance_portal is None
            or int(entrance_portal[1]) != 1
            or int(entrance_portal[4]) not in source_world_ids
        ):
            raise ValueError("The repair-source entrance portal is not reusable.")
        one_sided_portal = portal_without_opening_b(
            bytes(entrance_portal[7]), entrance["portalId"]
        )
        exit_saved = dict(exit_elevator["saved"])
        exit_saved.pop("portal", None)
        exit_blob = encode_lua_object_blob(exit_elevator["blob"], exit_saved)

        connection.execute("BEGIN IMMEDIATE")
        existing_portals = connection.execute(
            f"SELECT COUNT(*) FROM Portal WHERE id IN "
            f"({','.join('?' for _ in portal_ids)})",
            tuple(portal_ids),
        ).fetchone()[0]
        if existing_portals != 0:
            raise ValueError("Old Warehouse portal identifiers are already in use.")
        for elevator in (entrance, exit_elevator):
            if connection.execute(
                "SELECT 1 FROM ScriptData WHERE uid = ? AND key = ? LIMIT 1",
                (elevator["uid"], elevator["key"]),
            ).fetchone() is not None:
                raise ValueError("The failed elevator storage is no longer empty.")
            if connection.execute(
                "SELECT 1 FROM Controller WHERE id = ? LIMIT 1",
                (elevator["controllerId"],),
            ).fetchone() is None:
                raise ValueError("A physical Warehouse elevator controller is missing.")

        elevator_key = global_storage_key(ELEVATOR_STORAGE_CHANNEL)
        manager_row = connection.execute(
            "SELECT data FROM ScriptData WHERE uid = ? AND key = ? LIMIT 1",
            (GLOBAL_STORAGE_UID, elevator_key),
        ).fetchone()
        if manager_row is None:
            raise ValueError("The save has no elevator manager registry.")
        manager_blob = bytes(manager_row[0])
        manager = decode_lua_object(manager_blob)
        if not isinstance(manager, dict):
            raise ValueError("The elevator manager registry has an unsupported format.")
        manager = {
            key: item for key, item in manager.items()
            if not (
                key in portal_ids
                or (
                    isinstance(item, dict)
        ...
        manager_cursor = connection.execute(
            "UPDATE ScriptData SET data = ? WHERE uid = ? AND key = ? AND data = ?",
            (
                encode_lua_object_blob(manager_blob, manager),
                GLOBAL_STORAGE_UID, elevator_key, manager_blob,
            ),
        )
        if manager_cursor.rowcount != 1:
            raise RuntimeError("The elevator manager changed during repair.")
        connection.execute(
            "INSERT INTO Portal "
            "(id, worldIdA, xA, yA, worldIdB, xB, yB, data) "
            "VALUES (?, ?, ?, ?, 65535, 0, 0, ?)",
            (
                entrance_portal[0], entrance_portal[1],
                entrance_portal[2], entrance_portal[3], one_sided_portal,
            ),
        )
        connection.execute(
            ...
            "VALUES (?, ?, ?, ?, ?)",
            (
                entrance["uid"], entrance["key"], entrance["worldId"],
                entrance["flags"], entrance["blob"],
            ),
        )
        connection.execute(
            "INSERT INTO ScriptData (uid, key, worldId, flags, data) "
            "VALUES (?, ?, ?, ?, ?)",
            (
                exit_elevator["uid"], exit_elevator["key"],
                exit_elevator["worldId"], exit_elevator["flags"], exit_blob,
            ),
        )
        integrity = connection.execute("PRAGMA integrity_check").fetchone()
        if integrity is None or integrity[0] != "ok":
            raise ValueError("The repaired save did not pass SQLite integrity checking.")
        connection.commit()
    except Exception:
        connection.rollback()
        raise
    ...
    restoration.update({
        "elevatorStrategy": "one-sided-entrance-portal-v2",
        "entrancePortalId": entrance["portalId"],
        "elevatorRepairUtc": repaired_utc,
    })
    write_warehouse_restorations(save_identity, restorations)
    LOGGER.info(
        "Warehouse elevator repair result=ok warehouseIndex=%s portal=%s.",
        target_warehouse.get("index"), entrance["portalId"],
    )
    return {
        "featureId": feature_id,
        "repairedUtc": repaired_utc,
        "entrancePortalId": entrance["portalId"],
    }


def repair_revived_warehouse_emergency_exits(
    path: Path, source_path: Path, save_identity: str, feature_id: str
) -> dict:
    """Restore exterior emergency portals omitted by the v2 revival strategy."""
    match = re.fullmatch(r"regular-warehouse-(-?\d+)-(-?\d+)", feature_id)
    if match is None:
        raise ValueError("Invalid regular warehouse identifier.")
    cell_x, cell_y = (int(value) for value in match.groups())
    ...
    if not isinstance(restoration, dict):
        raise ValueError("This Warehouse has no recorded revival to repair.")
    if restoration.get("elevatorStrategy") == "exterior-portals-one-sided-v3":
        raise ValueError("This Warehouse already preserves its emergency exits.")
    if restoration.get("elevatorStrategy") != "one-sided-entrance-portal-v2":
        raise ValueError("This Warehouse is not in the supported v2 repair state.")
    entrance_portal_id = restoration.get("entrancePortalId")
    old_world_ids = set(restoration.get("orphanedWorldIds", []))
    if not isinstance(entrance_portal_id, int) or not old_world_ids or not all(
        isinstance(world_id, int) for world_id in old_world_ids
    ):
        raise ValueError("The recorded Warehouse revival is incomplete.")
    ...
    try:
        ...
        target_seed = connection.execute("SELECT seed FROM Game").fetchone()
        if source_seed is None or target_seed is None or source_seed != target_seed:
            raise ValueError("The emergency-exit repair source belongs to another seed.")
        ...
        if source_row is None:
            raise ValueError("The repair source has no Warehouse registry.")
        source_registry = decode_lua_object(bytes(source_row[0]))
        source_matches = [
            entry for entry in source_registry.values()
            if isinstance(entry, dict)
            and entry.get("zeroCell") == {"x": cell_x, "y": cell_y}
        ] if isinstance(source_registry, dict) else []
        if len(source_matches) != 1:
            raise ValueError("The emergency-exit repair target could not be matched.")
        source_warehouse = source_matches[0]
        source_world_ids = {
            item["id"] for item in source_warehouse.get("worlds", {}).values()
            if isinstance(item, dict)
            and item.get("type") == "world"
        ...
        if (
            source_warehouse.get("isQuestWarehouse") is not False
            or source_warehouse.get("destroyed") is not True
            or source_world_ids != old_world_ids
        ):
            raise ValueError("The source does not match the recorded destroyed Warehouse.")

        placeholders = ",".join("?" for _ in old_world_ids)
        exterior_rows = list(source.execute(
            f"SELECT id, worldIdA, xA, yA, worldIdB, xB, yB, data FROM Portal "
            f"WHERE worldIdA = 1 AND worldIdB IN ({placeholders})",
            tuple(old_world_ids),
        ))
        emergency_rows = [
            row for row in exterior_rows if int(row[0]) != entrance_portal_id
        ]
        if len(emergency_rows) != len(old_world_ids):
            raise ValueError("The source emergency-exit portal set is incomplete.")
        emergency_portals = {
            int(row[0]): portal_without_opening_b(bytes(row[7]), int(row[0]))
        ...
        connection.execute("BEGIN IMMEDIATE")
        entrance_row = connection.execute(
            "SELECT worldIdA, worldIdB, length(data) FROM Portal WHERE id = ?",
            (entrance_portal_id,),
        ).fetchone()
        if (
            entrance_row is None
            or int(entrance_row[0]) != 1
            or int(entrance_row[1]) in old_world_ids
            or int(entrance_row[2]) not in {96, 126}
        ):
            raise ValueError("The repaired Warehouse entrance portal is not intact.")
        existing = connection.execute(
            f"SELECT COUNT(*) FROM Portal WHERE id IN "
            f"({','.join('?' for _ in emergency_portals)})",
            tuple(emergency_portals),
        ).fetchone()[0]
        if existing != 0:
            raise ValueError("An emergency-exit portal identifier is already in use.")
        for row in emergency_rows:
            portal_id = int(row[0])
            connection.execute(
                "INSERT INTO Portal "
                "(id, worldIdA, xA, yA, worldIdB, xB, yB, data) "
                "VALUES (?, ?, ?, ?, 65535, 0, 0, ?)",
                (portal_id, int(row[1]), int(row[2]), int(row[3]),
                 emergency_portals[portal_id]),
            )
        integrity = connection.execute("PRAGMA integrity_check").fetchone()
        if integrity is None or integrity[0] != "ok":
            raise ValueError("The emergency-exit repair failed SQLite integrity checking.")
        connection.commit()
    except Exception:
        connection.rollback()
        raise
    ...
    repaired_utc = datetime.now(timezone.utc).isoformat()
    emergency_portal_ids = sorted(emergency_portals)
    restoration.update({
        "elevatorStrategy": "exterior-portals-one-sided-v3",
        "emergencyPortalIds": emergency_portal_ids,
        "emergencyExitRepairUtc": repaired_utc,
    })
    write_warehouse_restorations(save_identity, restorations)
    LOGGER.info(
        "Warehouse emergency-exit repair result=ok portals=%s.",
        emergency_portal_ids,
    )
    return {
        "featureId": feature_id,
        "repairedUtc": repaired_utc,
...
def read_underground_progression(path: Path, game_path: Path) -> dict:
    ...
    try:
        ...
        for depth, name, card_tier in UNDERGROUND_DEPTHS:
            ...
            terrain_3d = None
            if generated:
                terrain_document = summarize_world(path, world_id, game_path)
                terrain_filename = f"underground-terrain-world-{world_id}.json"
                terrain_target = GENERATED_DIR / terrain_filename
                terrain_target.parent.mkdir(parents=True, exist_ok=True)
                terrain_temporary = terrain_target.with_suffix(".json.tmp")
                terrain_temporary.write_text(json.dumps(terrain_document, separators=(",", ":")), encoding="utf-8")
                terrain_temporary.replace(terrain_target)
                terrain_3d = {
                    key: value for key, value in terrain_document.items()
                    if key not in {"tileUuids", "tilePaths", "caveCells", "pocketCells", "tunnels", "savedVoxelChangeCells", "authoredResourcePoints"}
                }
                terrain_3d["dataFile"] = terrain_filename
                voxel_cells = [
            ...
            floors.append({
                "depth": depth,
                "name": name,
                "floorKind": UNDERGROUND_FLOOR_KINDS[depth][0],
                "mineable": UNDERGROUND_FLOOR_KINDS[depth][1],
                "worldId": world_id,
                "generated": generated,
...
def rescue_vehicle_signature(shape_records: list[bytes]) -> bool:
    """Return whether a complete movable component is the exact Rescue Vehicle."""
    identifiers = [record[11:27] for record in shape_records if len(record) >= 27]
    expected = set(RESCUE_VEHICLE_UUID_BYTES.values())
    if len(identifiers) != len(shape_records) or set(identifiers) != expected:
        return False
    counts = Counter(identifiers)
    scrap_metal = RESCUE_VEHICLE_UUID_BYTES["scrapMetal"]
    try:
        metal_cells = sum(
            math.prod(struct.unpack("<3H", record[42:48]))
            for record in shape_records
            if record[11:27] == scrap_metal and len(record) >= 48
        )
    except struct.error:
        return False
    return (
        counts[RESCUE_VEHICLE_UUID_BYTES["scrapEngine"]] == 2
        and metal_cells == 7
        and counts[RESCUE_VEHICLE_UUID_BYTES["scrapWheel"]] == 5
        and counts[RESCUE_VEHICLE_UUID_BYTES["scrapSeat"]] == 1
        and counts[RESCUE_VEHICLE_UUID_BYTES["portableCraftbot"]] == 1
    )


def recycler_signature(shape_records: list[bytes]) -> bool:
    """Return whether one complete body is the exact Recycler construction."""
    identifiers = [record[11:27] for record in shape_records if len(record) >= 27]
    expected = set(RECYCLER_UUID_BYTES.values())
    if len(identifiers) != len(shape_records) or set(identifiers) != expected:
        return False
    counts = Counter(identifiers)
    metal_records = [
        record
        for record in shape_records
        if record[11:27] == RECYCLER_UUID_BYTES["scrapMetal"]
    ]
    wood_records = [
        record
        for record in shape_records
        if record[11:27] == RECYCLER_UUID_BYTES["scrapWood"]
    ]
    if any(len(record) < 48 for record in metal_records + wood_records):
        return False
    try:
        metal_cells = sum(
            math.prod(struct.unpack("<3H", record[42:48]))
    ...
    return (
        metal_cells == 7
        and wood_cells == 7
        and counts[RECYCLER_UUID_BYTES["portableCraftbot"]] == 1
        and counts[RECYCLER_UUID_BYTES["largeChest"]] == 3
        and counts[RECYCLER_UUID_BYTES["toiletPaper"]] == 2
    )


def large_chest_container_id(controller_data: bytes, host_id: int) -> int:
    """Decode the saved Large Chest controller's container reference."""
    if (
        len(controller_data) != 32
        or controller_data[:3] != b"\x03\x1b\x01"
        or int.from_bytes(controller_data[7:11], "big") != host_id
        or int.from_bytes(controller_data[12:16], "big") != host_id
        or controller_data[11] != 1
        or controller_data[16] != 1
    ):
        raise ValueError("A Recycler Large Chest controller has an unsupported format.")
    encoded = int.from_bytes(controller_data[-4:], "big")
    if encoded <= 0 or encoded & 0x7f:
        raise ValueError("A Recycler Large Chest container reference is invalid.")
    return encoded >> 7


def decode_container_blob(blob: bytes, expected_id: int) -> dict:
    """Decode the strict saved-container format used by the Recycler chests."""
    if (
        len(blob) < 13
        or blob[:3] != b"\x04\x00\x01"
        or int.from_bytes(blob[3:7], "big") != expected_id
        or blob[9:11] != b"\xff\xff"
        or blob[-2:] != b"\x00\x00"
    ):
        raise ValueError("A Recycler chest has an unsupported container format.")
    slot_count = int.from_bytes(blob[7:9], "big")
    if slot_count != 30 or len(blob) != 13 + slot_count * 22:
        raise ValueError("Each Recycler Large Chest must have exactly 30 saved slots.")
    slots = []
    for index in range(slot_count):
        record = blob[11 + index * 22:33 + index * 22]
        raw_uuid = record[:16]
        instance_id = int.from_bytes(record[16:20], "big", signed=True)
        quantity = int.from_bytes(record[20:22], "big")
        if raw_uuid == bytes(16):
            if quantity != 0:
                raise ValueError("A Recycler chest contains an invalid empty slot.")
            slots.append({"uuid": None, "instance": instance_id, "quantity": 0})
            continue
        if quantity <= 0:
            raise ValueError("A Recycler chest contains an invalid item quantity.")
        slots.append(
            {
                "uuid": str(uuid.UUID(bytes=raw_uuid[::-1])),
                "instance": instance_id,
                "quantity": quantity,
            }
        )
    return {"id": expected_id, "slotCount": slot_count, "slots": slots, "blob": blob}


def encode_container_blob(container: dict) -> bytes:
    encoded = bytearray(container["blob"])
    for index, slot in enumerate(container["slots"]):
        start = 11 + index * 22
        item_id = slot["uuid"]
        raw_uuid = bytes(16) if item_id is None else uuid.UUID(item_id).bytes[::-1]
        encoded[start:start + 16] = raw_uuid
        encoded[start + 16:start + 20] = int(slot["instance"]).to_bytes(
            4, "big", signed=True
        )
        encoded[start + 20:start + 22] = int(slot["quantity"]).to_bytes(2, "big")
    return bytes(encoded)


def read_saved_recyclers(
    path: Path,
    *,
    connection: sqlite3.Connection | None = None,
    include_internal: bool = False,
) -> dict:
    """Find exact Recycler bodies and their three linked Large Chests."""
    ...
    try:
        if owns_connection:
            connection.execute("PRAGMA query_only = ON")
        bodies = {
            int(body_id): {"worldId": int(world_id), "data": bytes(data)}
            for body_id, world_id, data in connection.execute(
                "SELECT id, worldId, data FROM RigidBody"
            )
        }
        shapes_by_body: dict[int, list[tuple[int, bytes]]] = {}
        for shape_id, body_id, data in connection.execute(
            "SELECT id, bodyId, data FROM ChildShape"
        ):
            shapes_by_body.setdefault(int(body_id), []).append((int(shape_id), bytes(data)))

        recyclers = []
        for body_id, shape_rows in shapes_by_body.items():
            body = bodies.get(body_id)
            records = [data for _, data in shape_rows]
            if (
                body is None
                or body["worldId"] != 1
                or len(body["data"]) <= 1
                or body["data"][1] != 2
                or not recycler_signature(records)
            ):
                continue
            bounds = connection.execute(
                "SELECT minX, maxX, minY, maxY FROM RigidBodyBounds WHERE id = ?",
                (body_id,),
            ).fetchone()
            if bounds is None or any(value is None for value in bounds):
                continue
            min_x, max_x, min_y, max_y = map(float, bounds)
            chest_shape_ids = sorted(
                shape_id
                for shape_id, data in shape_rows
                if data[11:27] == RECYCLER_UUID_BYTES["largeChest"]
            )
            containers = []
            operational = True
            error_message = None
            try:
                for shape_id in chest_shape_ids:
                    controllers = connection.execute(
                        "SELECT data FROM Controller WHERE hostId = ? AND hostType = 1",
                        (shape_id,),
                    ).fetchall()
                    if len(controllers) != 1:
                        raise ValueError(
                            "Each Recycler Large Chest must have one saved controller."
                        )
                    container_id = large_chest_container_id(
                        bytes(controllers[0][0]), shape_id
                    )
                    row = connection.execute(
                        "SELECT data FROM Container WHERE id = ?", (container_id,)
                    ).fetchone()
                    if row is None:
                        raise ValueError("A Recycler Large Chest container is missing.")
                    container = decode_container_blob(bytes(row[0]), container_id)
                    container["shapeId"] = shape_id
                    containers.append(container)
                if len({item["id"] for item in containers}) != 3:
                    raise ValueError("Recycler Large Chests must use three distinct containers.")
            except ValueError as error:
                operational = False
                error_message = str(error)
                containers = []
            recycler = {
                "id": f"recycler-{body_id}",
                "x": (min_x + max_x) / 2,
                "y": (min_y + max_y) / 2,
                "worldId": 1,
                "details": "Recycler",
                ...
                "slotCount": 90,
                "occupiedSlots": sum(
                    slot["uuid"] is not None
                    for container in containers
                    for slot in container["slots"]
                ),
                "operational": operational,
                "message": error_message or "Exact Recycler signature and three Large Chests verified.",
                "_bodyId": body_id,
                "_containers": containers,
            }
            if not include_internal:
                recycler = {
                    key: value for key, value in recycler.items() if not key.startswith("_")
                }
            recyclers.append(recycler)
        recyclers.sort(key=lambda item: item["id"])
        return {
            "status": "available",
            "recyclers": recyclers,
            "count": len(recyclers),
            "message": f"Found {len(recyclers)} exact Recycler construction(s).",
        }
        ...
        return {
            ...
            "recyclers": [],
            "count": 0,
            "message": f"Recycler records are unavailable: {error}",
        }
    finally:
        if owns_connection:
            connection.close()
...
def resolve_recycler_resources(
    item_id: str,
    catalog: dict,
    ...
    if item_id in memo:
        return memo[item_id].copy()
    if item_id in trail:
        raise ValueError("A recursive recipe cycle was detected.")
    if item_id in catalog["ambiguous"]:
        raise ValueError("This item has multiple different crafting recipes.")
    recipe = catalog["recipes"].get(item_id)
    if recipe is None:
        result: Counter[str] = Counter({item_id: Fraction(1, 1)})
        memo[item_id] = result
        return result.copy()
    output_quantity, ingredients = recipe
    # Several Chapter 2 resource recipes consume one unit of their own output
    # as a catalyst. Those resources are terminal Recycler materials; expanding
    # them would create a false recursive cycle.
    if any(ingredient_id == item_id for ingredient_id, _ in ingredients):
        result = Counter({item_id: Fraction(1, 1)})
        memo[item_id] = result
        return result.copy()
    result: Counter[str] = Counter()
    for ingredient_id, quantity in ingredients:
        resolved = resolve_recycler_resources(
            ingredient_id, catalog, memo, trail + (item_id,)
        )
        factor = Fraction(quantity, output_quantity)
        for resource_id, resource_quantity in resolved.items():
            result[resource_id] += resource_quantity * factor
    memo[item_id] = result
    return result.copy()


def recycler_item_summary(quantities: Counter[str], catalog: dict) -> list[dict]:
    return [
        {
            "itemId": item_id,
            "name": catalog["names"].get(item_id, item_id),
            "quantity": int(quantity),
        }
        for item_id, quantity in sorted(
            quantities.items(), key=lambda item: catalog["names"].get(item[0], item[0])
        )
        if quantity > 0
    ]


def build_recycler_preview(
    path: Path,
    game_path: Path,
    recycler_id: str,
    *,
    ...
    try:
        if owns_connection:
            connection.execute("PRAGMA query_only = ON")
        result = read_saved_recyclers(
            path, connection=connection, include_internal=True
        )
        if result["status"] != "available":
            raise ValueError(result["message"])
        if len(result["recyclers"]) != 1:
            raise ValueError(
                f"Expected one exact Recycler, found {len(result['recyclers'])}."
            )
        recycler = result["recyclers"][0]
        if recycler["id"] != recycler_id:
            raise ValueError("The selected Recycler changed after the map loaded.")
        if not recycler["operational"]:
            raise ValueError(recycler["message"])

        catalog = load_recycler_catalog(game_path)
        containers = []
        for original in sorted(recycler["_containers"], key=lambda item: item["id"]):
            containers.append(
                {
                    **original,
                    "slots": [dict(slot) for slot in original["slots"]],
                }
            )

        input_quantities: Counter[str] = Counter()
        unsupported: dict[tuple[str, str], int] = Counter()
        eligible_slots = []
        ...
        for container in containers:
            for slot_index, slot in enumerate(container["slots"]):
                ...
                if slot["instance"] != -1:
                    unsupported[(item_id, "Special item data is not supported.")] += quantity
                    continue
                if item_id in catalog["ambiguous"]:
                    unsupported[(item_id, "Multiple different recipes are available.")] += quantity
                    continue
                if item_id not in catalog["recipes"]:
                    unsupported[(item_id, "No supported crafting recipe was found.")] += quantity
                    continue
                try:
                    resolved = resolve_recycler_resources(item_id, catalog, memo)
                except ValueError as error:
                    unsupported[(item_id, str(error))] += quantity
                    continue
                input_quantities[item_id] += quantity
                eligible_slots.append((container, slot_index))
                for resource_id, resource_quantity in resolved.items():
                    resource_totals[resource_id] += resource_quantity * quantity

        returned_quantities: Counter[str] = Counter()
        for resource_id, quantity in resource_totals.items():
            recovered = quantity * RECYCLER_RECOVERY_RATE
            returned_quantities[resource_id] = recovered.numerator // recovered.denominator
        returned_quantities += Counter()

        for container, slot_index in eligible_slots:
        ...
        capacity_error = None
        for item_id in sorted(returned_quantities):
            remaining = returned_quantities[item_id]
            if remaining <= 0:
                continue
            stack_size = catalog["stackSizes"].get(item_id)
            if stack_size is None:
                capacity_error = (
                    f"The stack limit for {catalog['names'].get(item_id, item_id)} is unavailable."
                )
                break
            for container in containers:
                for slot in container["slots"]:
                    if (
                        slot["uuid"] == item_id
                        and slot["instance"] == -1
                        and slot["quantity"] < stack_size
                    ):
                        added = min(remaining, stack_size - slot["quantity"])
                        slot["quantity"] += added
                        remaining -= added
                        if remaining == 0:
                            break
            ...
            if remaining:
                for container in containers:
                    for index, slot in enumerate(container["slots"]):
                        if slot["uuid"] is None:
                            added = min(remaining, stack_size)
            ...
            if remaining:
                capacity_error = (
                    f"The three Recycler chests need more space for "
                    f"{catalog['names'].get(item_id, item_id)}."
                )
                break

        positive_returns = sum(returned_quantities.values()) > 0
        can_recycle = bool(eligible_slots) and positive_returns and capacity_error is None
        if not eligible_slots:
            message = "No recyclable items were found in the three Large Chests."
        elif not positive_returns:
            message = "The eligible quantity is too small to produce a 50% return."
        elif capacity_error:
            message = capacity_error + " Nothing will be changed."
        else:
            message = (
                f"Ready to recycle {sum(input_quantities.values())} item(s) at a 50% return "
                "across all three Large Chests."
            )

        preview_hash = hashlib.sha256()
        preview_hash.update(recycler_id.encode("utf-8"))
        preview_hash.update(catalog["hash"].encode("ascii"))
        for container in containers:
            preview_hash.update(container["id"].to_bytes(4, "big"))
            preview_hash.update(container["blob"])
        preview_hash.update(
            json.dumps(sorted(returned_quantities.items()), separators=(",", ":")).encode("utf-8")
        )
        occupied_before = recycler["occupiedSlots"]
        occupied_after = sum(
            slot["uuid"] is not None
            for container in containers
            for slot in container["slots"]
        ...
        preview = {
            ...
            "message": message,
            "inputItems": recycler_item_summary(input_quantities, catalog),
            "returnedItems": recycler_item_summary(returned_quantities, catalog),
            "unsupportedItems": [
                {
                    "itemId": item_id,
                    "name": catalog["names"].get(item_id, item_id),
                ...
                for (item_id, reason), quantity in sorted(
                    unsupported.items(),
                    key=lambda item: catalog["names"].get(item[0][0], item[0][0]),
                )
            ],
            "capacity": {
                "totalSlots": 90,
                "occupiedBefore": occupied_before,
                "occupiedAfter": occupied_after if can_recycle else occupied_before,
            },
            "previewToken": preview_hash.hexdigest(),
            "_updates": [
                {
                    "id": container["id"],
                    "original": container["blob"],
                    "updated": encode_container_blob(container),
                }
                for container in containers
            ] if can_recycle else [],
        }
...
def recycle_saved_recycler(
    path: Path, game_path: Path, recycler_id: str, preview_token: str
) -> dict:
    """Apply one already-previewed Recycler transaction atomically."""
    connection = sqlite3.connect(path, timeout=5)
    try:
        connection.execute("BEGIN IMMEDIATE")
        preview = build_recycler_preview(
            path,
            game_path,
            recycler_id,
        ...
        if preview["previewToken"] != preview_token:
            raise ValueError(
                "The Recycler or its chest contents changed after the preview. Preview again."
            )
        if not preview["canRecycle"]:
            raise ValueError(preview["message"])
        for update in preview["_updates"]:
            changed = connection.execute(
                "UPDATE Container SET data = ? WHERE id = ? AND data = ?",
                (update["updated"], update["id"], update["original"]),
            )
            if changed.rowcount != 1:
                raise ValueError("A Recycler chest changed during the transaction.")
        integrity = connection.execute("PRAGMA integrity_check").fetchone()
        if integrity is None or integrity[0] != "ok":
            raise ValueError("The edited save did not pass SQLite integrity checking.")
        connection.commit()
        LOGGER.info(
            "Recycler result=ok recycler=%s inputItems=%s returnedItems=%s.",
            recycler_id,
            sum(item["quantity"] for item in preview["inputItems"]),
            sum(item["quantity"] for item in preview["returnedItems"]),
        )
        return {
            key: value for key, value in preview.items() if not key.startswith("_")
        }
...
def saved_rigid_body_position(data: bytes) -> tuple[float, float, float]:
    """Decode the authoritative XYZ reference position of a movable body."""
    if len(data) < 55 or data[1] != 2:
        raise ValueError("A selected vehicle body has an unsupported save format.")
    return struct.unpack(">3f", data[43:55])


def translate_saved_rigid_body(
    data: bytes, dx: float, dy: float, dz: float
) -> bytes:
    """Translate one body while preserving its rotation and all other fields."""
    translated = bytearray(data)
    max_y, min_y, max_x, min_x = struct.unpack(">4f", translated[9:25])
    translated[9:25] = struct.pack(
        ">4f", max_y + dy, min_y + dy, max_x + dx, min_x + dx
    )
    x, y, z = saved_rigid_body_position(translated)
    translated[43:55] = struct.pack(">3f", x + dx, y + dy, z + dz)
    return bytes(translated)


def read_saved_vehicles(
    path: Path,
    *,
    connection: sqlite3.Connection | None = None,
    include_internal: bool = False,
    ...
    try:
        ...
        shape_bodies = {}
        shape_records_by_body: dict[int, list[bytes]] = {}
        for shape_id, body_id, data in connection.execute(
            "SELECT id, bodyId, data FROM ChildShape"
        ):
            shape_bodies[int(shape_id)] = int(body_id)
            shape_records_by_body.setdefault(int(body_id), []).append(bytes(data))
        ...
        for body_id, records in shape_records_by_body.items():
            for data in records:
                if len(data) < 27:
                    continue
        ...
        for candidate_body in candidate_bodies:
            ...
            component_shape_records = [
                record
                for body_id in component
                for record in shape_records_by_body.get(body_id, ())
            ]
            ...
            is_rescue_vehicle = rescue_vehicle_signature(component_shape_records)
            if is_rescue_vehicle:
                details = "Rescue Vehicle"
            try:
                positions = [
                    saved_rigid_body_position(bodies[body_id]["data"])
                    for body_id in component
                ]
            ...
            vehicles.append(
                {
                    ...
                    "details": details,
                    "isRescueVehicle": is_rescue_vehicle,
                    "_bodyIds": sorted(component),
                    "_reference": component_reference,
                }
            )
...
def recover_saved_vehicle(path: Path, vehicle_id: str) -> dict:
    """Move one current vehicle component seven metres above the Rescue Vehicle."""
    connection = sqlite3.connect(path, timeout=5)
    try:
        connection.execute("BEGIN IMMEDIATE")
        result = read_saved_vehicles(
            path, connection=connection, include_internal=True
        )
        if result["status"] != "available":
            raise ValueError(result["message"])
        vehicles = {vehicle["id"]: vehicle for vehicle in result["vehicles"]}
        selected = vehicles.get(vehicle_id)
        if selected is None:
            raise ValueError("The selected vehicle changed after the map loaded.")
        rescue_vehicles = [
            vehicle
            for vehicle in result["vehicles"]
            if vehicle["isRescueVehicle"]
        ]
        if len(rescue_vehicles) != 1:
            raise ValueError(
                f"Expected one exact Rescue Vehicle, found {len(rescue_vehicles)}."
            )
        rescue = rescue_vehicles[0]
        if selected["id"] == rescue["id"]:
            raise ValueError("The Rescue Vehicle cannot recover itself.")
        selected_reference = selected["_reference"]
        rescue_reference = rescue["_reference"]
        if selected_reference is None or rescue_reference is None:
            raise ValueError("A required vehicle position has an unsupported format.")
        dx = rescue_reference[0] - selected_reference[0]
        dy = rescue_reference[1] - selected_reference[1]
        dz = rescue_reference[2] + 7.0 - selected_reference[2]
        if not all(math.isfinite(value) for value in (dx, dy, dz)):
            raise ValueError("The calculated recovery position is invalid.")
        for body_id in selected["_bodyIds"]:
            row = connection.execute(
                "SELECT data FROM RigidBody WHERE id = ?", (body_id,)
            ).fetchone()
            if row is None:
                raise ValueError("A selected vehicle body no longer exists.")
            translated = translate_saved_rigid_body(bytes(row[0]), dx, dy, dz)
            updated = connection.execute(
                "UPDATE RigidBody SET data = ? WHERE id = ?",
                (translated, body_id),
            )
            if updated.rowcount != 1:
                raise ValueError("A selected vehicle body could not be updated.")
            bounds_updated = connection.execute(
                "UPDATE RigidBodyBounds "
                "SET minX = minX + ?, maxX = maxX + ?, "
                "minY = minY + ?, maxY = maxY + ? WHERE id = ?",
                (dx, dx, dy, dy, body_id),
            )
            if bounds_updated.rowcount != 1:
                raise ValueError("A selected vehicle spatial record is missing.")
        integrity = connection.execute("PRAGMA integrity_check").fetchone()
        if integrity is None or integrity[0] != "ok":
            raise ValueError("The edited save did not pass SQLite integrity checking.")
        connection.commit()
        LOGGER.info(
            "Vehicle recovery result=ok vehicle=%s bodyCount=%s rescue=%s.",
            selected["id"],
            len(selected["_bodyIds"]),
            rescue["id"],
        )
        return {
            "vehicleId": selected["id"],
            "rescueVehicleId": rescue["id"],
            "bodyCount": len(selected["_bodyIds"]),
        }
    except Exception:
        connection.rollback()
        raise
...
def build_state(game_path: Path, user_dirs: list[Path], saves: list[Path], selected: Path) -> dict:
    ...
    warehouse_restoration = read_saved_warehouses(
        selected, save_data["identity"]
    )
    ...
    saved_recyclers = read_saved_recyclers(selected)
    underground = read_underground_progression(selected, game_path)
    excavation_island = build_excavation_island_map(
        selected,
        game_path,
        saved_beacons,
    )
    ...
    for feature in confirmed_map["features"]:
        restoration = warehouse_restoration["warehouses"].get(feature["id"])
        if restoration is not None:
            feature["warehouseRestoration"] = restoration
    ...
    state = {
        ...
        "save": save_data,
        "warehouseRestoration": warehouse_restoration,
        "map": {
            "cellSizeMeters": CELL_SIZE_METERS,
            "northUp": True,
            ...
            **confirmed_map,
            ...
            "recyclers": saved_recyclers,
        },
        "playerPosition": saved_position,
        "underground": underground,
        "excavationIsland": excavation_island,
        "layers": [
            ...
            },
            {
                "id": "recyclers",
                "label": "Recycler",
                "available": saved_recyclers["status"] == "available"
                and saved_recyclers["count"] > 0,
                "visible": saved_recyclers["status"] == "available"
                and saved_recyclers["count"] > 0,
            },
        ...
        "diagnostics": [
            ...
            {
                "level": (
                    "ok" if saved_recyclers["status"] == "available" else "warning"
                ),
                "message": saved_recyclers["message"],
            },
            {
...
class ScrapMapSession:
    ...
    def generate(self) -> dict:
        with self.lock:
            ...
            self.selected_identity = state["save"]["identity"]
            write_state(state)
            return state
    ...
    def update_player_position(self, identity: str) -> dict:
        with self.lock:
            if identity != self.selected_identity:
                raise ValueError(
                    "The selected save changed after the player marker was selected."
                )
            position = read_player_position_from_raw_snapshot(self.selected)
            LOGGER.info(
                "Player-only snapshot identity=%s status=%s.",
                self.selected_identity,
                position["status"],
            )
            return {
                "identity": self.selected_identity,
                "filename": self.selected.name,
    ...
    def recover_vehicle(self, identity: str, vehicle_id: str) -> dict:
        with self.lock:
            current_identity = read_save_identity(self.selected)["id"]
            if identity != current_identity:
                raise ValueError(
                    "The selected save changed after the vehicle was selected."
                )
            recover_saved_vehicle(self.selected, vehicle_id)
            state = build_state(
                self.game_path, self.user_dirs, self.saves, self.selected
            )
            write_state(state)
    ...
    def revive_warehouse(self, identity: str, feature_id: str) -> dict:
        with self.lock:
            current_identity = read_save_identity(self.selected)["id"]
            if identity != current_identity:
                raise ValueError(
                    "The selected save changed after the warehouse was selected."
                )
            revive_saved_warehouse(self.selected, current_identity, feature_id)
            state = build_state(
                self.game_path, self.user_dirs, self.saves, self.selected
            )
            self.selected_identity = state["save"]["identity"]
    ...
    def preview_recycler(self, identity: str, recycler_id: str) -> dict:
        with self.lock:
            current_identity = read_save_identity(self.selected)["id"]
            if identity != current_identity:
                raise ValueError(
                    "The selected save changed after the Recycler was selected."
                )
            return build_recycler_preview(
                self.selected, self.game_path, recycler_id
            )

    def recycle_recycler(
        self, identity: str, recycler_id: str, preview_token: str
    ) -> dict:
        with self.lock:
            current_identity = read_save_identity(self.selected)["id"]
            if identity != current_identity:
                raise ValueError(
                    "The selected save changed after the Recycler preview."
                )
            recycle_saved_recycler(
                self.selected, self.game_path, recycler_id, preview_token
            )
            state = build_state(
                self.game_path, self.user_dirs, self.saves, self.selected
            )
    ...
    class ScrapMapRequestHandler(SimpleHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            if self.path not in {
                ...
                "/api/delete-marker",
                "/api/recover-vehicle",
                "/api/revive-warehouse",
                "/api/update-player-position",
                "/api/recycler-preview",
                "/api/recycle",
            }:
                self.send_error(404)
                return
            try:
                ...
                if self.path == "/api/update-player-position":
                    identity = payload.get("identity")
                    if not isinstance(identity, str):
                        raise ValueError("Missing save identity.")
                    result = session.update_player_position(identity)
                    response = json.dumps({"ok": True, **result}).encode("utf-8")
                    self.send_response(200)
                    self.send_header("Content-Type", "application/json; charset=utf-8")
                    self.send_header("Cache-Control", "no-store")
                    self.send_header("Content-Length", str(len(response)))
                    ...
                    return
                if self.path == "/api/recycler-preview":
                    identity = payload.get("identity")
                    recycler_id = payload.get("recyclerId")
                    if not isinstance(identity, str) or not isinstance(
                        recycler_id, str
                    ):
                        raise ValueError("Invalid Recycler preview request.")
                    result = session.preview_recycler(identity, recycler_id)
                    response = json.dumps({"ok": True, **result}).encode("utf-8")
                    self.send_response(200)
                    self.send_header("Content-Type", "application/json; charset=utf-8")
                    self.send_header("Cache-Control", "no-store")
                    ...
                    self.wfile.write(response)
                    LOGGER.info(
                        "API action=%s result=ok identity=%s recycler=%s.",
                        self.path,
                        identity,
                        recycler_id,
                    )
                ...
                if self.path == "/api/refresh":
                ...
                elif self.path == "/api/delete-marker":
                    marker_id = payload.get("id")
                    if not isinstance(marker_id, str):
                        raise ValueError("Missing marker identifier.")
                    state = session.delete_marker(marker_id)
                elif self.path == "/api/recover-vehicle":
                    identity = payload.get("identity")
                    vehicle_id = payload.get("vehicleId")
                    if not isinstance(identity, str) or not isinstance(
                        vehicle_id, str
                    ):
                        raise ValueError("Invalid vehicle recovery request.")
                    state = session.recover_vehicle(identity, vehicle_id)
                elif self.path == "/api/revive-warehouse":
                    identity = payload.get("identity")
                    feature_id = payload.get("featureId")
                    if not isinstance(identity, str) or not isinstance(
                        feature_id, str
                    ):
                        raise ValueError("Invalid warehouse restoration request.")
                    state = session.revive_warehouse(identity, feature_id)
                else:
                    identity = payload.get("identity")
                    recycler_id = payload.get("recyclerId")
                    ...
                    if (
                        ...
                        or not isinstance(recycler_id, str)
                        or not isinstance(preview_token, str)
                    ):
                        raise ValueError("Invalid Recycler transaction request.")
                    state = session.recycle_recycler(
                        identity, recycler_id, preview_token
                    )
                response = json.dumps(
                    {
                        "ok": True,
