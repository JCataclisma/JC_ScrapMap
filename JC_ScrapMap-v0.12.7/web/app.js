...
function drawRecyclers() {
  if (!layerVisibility.get("recyclers")) return;
  const recyclers = state?.map?.recyclers?.recyclers || [];
  context.save();
  for (const recycler of recyclers) {
    const screen = worldToScreen(recycler.x, recycler.y);
    context.fillStyle = recycler.operational ? "#6fcf97" : "#eb5757";
    context.strokeStyle = "#07140c";
    context.lineWidth = 2;
    context.beginPath();
    for (let index = 0; index < 6; index += 1) {
      const angle = Math.PI / 3 * index - Math.PI / 2;
      const x = screen.x + Math.cos(angle) * 10;
      const y = screen.y + Math.sin(angle) * 10;
    ...
    if (viewWidthMeters <= 3000) {
      ...
      context.textAlign = "center";
      context.fillStyle = recycler.operational ? "#c9f7d9" : "#ffb5b5";
      context.fillText("Recycler", screen.x, screen.y - 16);
    }
  }
  context.restore();
}
...
function addDetail(term, value, valueId = null) {
  const list = document.getElementById("saveDetails");
  const dt = document.createElement("dt");
  const dd = document.createElement("dd");
  dt.textContent = term;
  dd.textContent = value;
  if (valueId) dd.id = valueId;
  list.append(dt, dd);
}
...
function updatePlayerInterface() {
  ...
  const savedPlayerDetail = document.getElementById("savedPlayerDetail");
  const playerCellDetail = document.getElementById("playerCellDetail");
  const playerSavedDetail = document.getElementById("playerSavedDetail");
  if (player && savedPlayerDetail && playerCellDetail && playerSavedDetail) {
    savedPlayerDetail.textContent =
      `X ${player.x.toFixed(1)} · Y ${player.y.toFixed(1)} · Z ${player.z.toFixed(1)}`;
    playerCellDetail.textContent =
      `${player.cellX}, ${player.cellY} · world ${player.worldId}`;
    playerSavedDetail.textContent = new Date(position.savedUtc).toLocaleString();
  }
  ...
  document.getElementById("playerStatus").textContent =
    `${playerMessage} Save file updated ${formatAge(position.savedUtc)}.`;
...
function populateInterface() {
  ...
  if (player) {
    ...
    addDetail(
      "Position saved",
      new Date(state.playerPosition.savedUtc).toLocaleString(),
      "playerSavedDetail",
    );
  }
...
canvas.addEventListener("click", (event) => {
  if (!state) return;
  hideWarehouseRevival();
  const rect = canvas.getBoundingClientRect();
  const world = screenToWorld(event.clientX - rect.left, event.clientY - rect.top);
  const player = state.playerPosition?.player;
  if (layerVisibility.get("player") && player?.worldId === 1) {
    const playerScreen = worldToScreen(player.x, player.y);
    const playerHit = Math.hypot(
      playerScreen.x - (event.clientX - rect.left),
      playerScreen.y - (event.clientY - rect.top),
    ) <= 12;
    if (playerHit) {
      hideInstantRecovery();
      hideRecycler();
      ...
      document.getElementById("updatePlayerPositionButton").hidden = false;
      document.getElementById("mapStatus").textContent =
        `Saved player position at X ${player.x.toFixed(1)} · Y ${player.y.toFixed(1)} · Z ${player.z.toFixed(1)}. Select Update saved player position to read a new local snapshot.`;
      return;
    }
  }
  ...
  const recycler = [...(state.map.recyclers?.recyclers || [])].reverse().find((item) => {
    if (!layerVisibility.get("recyclers")) return false;
    const screen = worldToScreen(item.x, item.y);
    return Math.hypot(screen.x - (event.clientX - rect.left), screen.y - (event.clientY - rect.top)) <= 13;
  });
  if (recycler) {
    hidePlayerUpdate();
    hideInstantRecovery();
    ...
    recyclerPreview = null;
    document.getElementById("discoveryButton").hidden = true;
    document.getElementById("excavationMapButton").hidden = true;
    document.getElementById("mapStatus").textContent =
      `Recycler saved at X ${recycler.x.toFixed(1)} · Y ${recycler.y.toFixed(1)}. ` +
      `${recycler.occupiedSlots} of ${recycler.slotCount} pooled chest slots are occupied. ${recycler.message}`;
    const details = document.getElementById("recyclerDetails");
    details.hidden = false;
    details.open = false;
    document.getElementById("recyclerPreview").hidden = true;
    document.getElementById("recyclerExecuteButton").hidden = true;
    document.getElementById("recyclerPreviewButton").disabled = !recycler.operational;
    return;
  }
  ...
  const warehouse = feature.warehouseRestoration;
  if (warehouse?.destroyed || warehouse?.restoredOnce) {
    const details = document.getElementById("warehouseRevivalDetails");
    const button = document.getElementById("warehouseRevivalButton");
    details.hidden = false;
    details.open = false;
    button.disabled = !warehouse.eligible;
    button.textContent = warehouse.eligible
      ? "REVIVE WAREHOUSE ONCE"
      : "ONE REVIVAL ALREADY USED";
    document.getElementById("warehouseRevivalMessage").textContent = warehouse.eligible
      ? "Scrap Mechanic must be closed. Restores the exterior and creates fresh interior floors on the next entry. This warehouse can be revived only once."
      : "This warehouse has already used its one revival and cannot be revived again.";
  }
...
function hideRecycler() {
  selectedRecycler = null;
  recyclerPreview = null;
  const details = document.getElementById("recyclerDetails");
  details.hidden = true;
  details.open = false;
  const preview = document.getElementById("recyclerPreview");
  preview.hidden = true;
  preview.textContent = "";
  const previewButton = document.getElementById("recyclerPreviewButton");
  previewButton.disabled = false;
  previewButton.textContent = "Preview recycling";
  const executeButton = document.getElementById("recyclerExecuteButton");
  executeButton.hidden = true;
  executeButton.disabled = true;
}

function hideWarehouseRevival() {
  const details = document.getElementById("warehouseRevivalDetails");
  details.hidden = true;
  details.open = false;
  const button = document.getElementById("warehouseRevivalButton");
  button.disabled = false;
  button.textContent = "REVIVE WAREHOUSE ONCE";
}

function hidePlayerUpdate() {
  playerSelected = false;
  const button = document.getElementById("updatePlayerPositionButton");
  button.hidden = true;
  button.disabled = false;
  button.textContent = "Update saved player position";
}
...
document.getElementById("updatePlayerPositionButton").addEventListener("click", async () => {
  if (!playerSelected || !state?.save?.identity) return;
  const button = document.getElementById("updatePlayerPositionButton");
  button.disabled = true;
  button.textContent = "Updating saved position…";
  try {
    const response = await fetch("../api/update-player-position", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ identity: state.save.identity }),
    });
    const result = await response.json();
    if (!response.ok || !result.ok) {
      throw new Error(result.error || `HTTP ${response.status}`);
    }
    if (result.identity !== state.save.identity) {
      throw new Error("The selected save changed; reopen the map and try again.");
    }
    if (result.playerPosition?.status !== "available") {
      throw new Error(result.playerPosition?.message || "Saved player position is unavailable.");
    }
    state.playerPosition = result.playerPosition;
    updatePlayerInterface();
    render();
    const player = state.playerPosition.player;
    document.getElementById("mapStatus").textContent =
      `Saved player position updated to X ${player.x.toFixed(1)} · Y ${player.y.toFixed(1)} · Z ${player.z.toFixed(1)} without reloading the map.`;
    if (player.worldId !== 1) hidePlayerUpdate();
  } catch (error) {
    document.getElementById("mapStatus").textContent =
      `Could not update saved player position: ${error.message}`;
  } finally {
    if (!button.hidden) {
      button.disabled = false;
      button.textContent = "Update saved player position";
...
function recyclerPreviewText(preview) {
  const lines = [preview.message, ""];
  lines.push("Recycling:");
  if (preview.inputItems.length) {
    preview.inputItems.forEach((item) => lines.push(`- ${item.quantity} x ${item.name}`));
  } else {
    lines.push("- Nothing eligible");
  }
  lines.push("", "50% return:");
  if (preview.returnedItems.length) {
    preview.returnedItems.forEach((item) => lines.push(`- ${item.quantity} x ${item.name}`));
  } else {
    lines.push("- Nothing");
  }
  if (preview.unsupportedItems.length) {
    lines.push("", "Left untouched:");
    preview.unsupportedItems.forEach((item) => {
      lines.push(`- ${item.quantity} x ${item.name}: ${item.reason}`);
    });
  }
  lines.push(
    "",
    `Pooled chest slots: ${preview.capacity.occupiedBefore}/${preview.capacity.totalSlots} before` +
      (preview.canRecycle ? `, ${preview.capacity.occupiedAfter}/${preview.capacity.totalSlots} after` : ""),
  );
  return lines.join("\n");
}

document.getElementById("recyclerPreviewButton").addEventListener("click", async () => {
  if (!selectedRecycler || !state?.save?.identity) return;
  const button = document.getElementById("recyclerPreviewButton");
  const previewElement = document.getElementById("recyclerPreview");
  const executeButton = document.getElementById("recyclerExecuteButton");
  button.disabled = true;
  button.textContent = "Reading all three chests…";
  executeButton.hidden = true;
  executeButton.disabled = true;
  recyclerPreview = null;
  try {
    const response = await fetch("../api/recycler-preview", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    recyclerPreview = result;
    previewElement.textContent = recyclerPreviewText(result);
    previewElement.hidden = false;
    executeButton.hidden = !result.canRecycle;
    executeButton.disabled = !result.canRecycle;
    button.textContent = "Refresh recycling preview";
  } catch (error) {
    previewElement.textContent = `Could not preview recycling: ${error.message}`;
    previewElement.hidden = false;
    button.textContent = "Preview recycling";
  } finally {
    button.disabled = false;
  }
});

document.getElementById("recyclerExecuteButton").addEventListener("click", async () => {
  if (!selectedRecycler || !recyclerPreview?.canRecycle || !state?.save?.identity) return;
  const executeButton = document.getElementById("recyclerExecuteButton");
  const previewButton = document.getElementById("recyclerPreviewButton");
  executeButton.disabled = true;
  ...
  try {
    const response = await fetch("../api/recycle", {
      ...
      body: JSON.stringify({
        identity: state.save.identity,
        recyclerId: selectedRecycler.id,
        previewToken: recyclerPreview.previewToken,
      }),
    });
    const result = await response.json();
    if (!response.ok || !result.ok) {
    ...
    executeButton.hidden = true;
    previewButton.disabled = false;
    document.getElementById("recyclerPreview").textContent =
      `Recycling was cancelled without changes: ${error.message}`;
  }
});
...
document.getElementById("warehouseRevivalButton").addEventListener("click", async () => {
  if (!selectedFeature?.warehouseRestoration?.eligible || !state?.save?.identity) return;
  const button = document.getElementById("warehouseRevivalButton");
  button.disabled = true;
  button.textContent = "REVIVING WAREHOUSE…";
  sessionStorage.setItem(
    `jc-scrapmap-view-${state.save.identity}`,
    JSON.stringify({ center, viewWidthMeters }),
  );
  try {
    const response = await fetch("../api/revive-warehouse", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        identity: state.save.identity,
    ...
    button.textContent = "REVIVE WAREHOUSE ONCE";
    document.getElementById("mapStatus").textContent =
      `Could not revive warehouse: ${error.message}`;
  }
});
