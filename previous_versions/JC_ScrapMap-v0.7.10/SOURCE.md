...
JC ScrapMap 0.7.7 ships as readable source code.
...
- `game-side/terrain_road_export.lua`

The Lua exporter reads generated roads, terrain-region flags, and exact
Schematic Station POI cells once, then writes a compact JSON result into a
temporary ownership-marked directory under `$SURVIVAL_DATA`. It does not call
`sm.storage.save` or `sm.terrainData.save`.
