...
def load_exact_roads(save_data: dict) -> dict:
    ...
    if not path.is_file():
        captured_seeds = sorted(
            {
                int(match.group(1))
                for candidate in IMPORT_DIR.glob("roads-*.json")
                if (match := re.fullmatch(r"roads-(-?\d+)\.json", candidate.name))
            }
        )
        other_seeds = [seed for seed in captured_seeds if seed != int(save_data["seed"])]
        message = "Exact roads have not been generated for this save yet."
        if other_seeds:
            message += (
                " Road data exists for another world. Select the save whose seed "
                f"matches one of these captures: {', '.join(map(str, other_seeds))}."
            )
        return {
            "status": "unavailable",
            "seed": save_data["seed"],
...
class ScrapMapSession:
    ...
    def select_seed(self, seed: int) -> dict:
        with self.lock:
            ...
            if candidate is None:
                raise ValueError(
                    f"Roads were captured for seed {seed}, but no matching save "
                    "was found. Keep the save in the normal Survival save folder "
                    "and choose Open map again."
                )
            self.selected = candidate
            state = build_state(
                self.game_path, self.user_dirs, self.saves, self.selected
            ...
            try:
                ...
                if self.path == "/api/refresh":
                    ...
                    if not isinstance(seed, int):
                        raise ValueError("Missing captured world seed.")
