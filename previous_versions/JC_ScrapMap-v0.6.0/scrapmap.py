...
START_REGION_BOUNDS = {"xMin": -48, "xMax": -20, "yMin": -48, "yMax": -18}
...
def inspect_generator_confirmed_map(game_path: Path) -> dict:
    ...
    if not generator_path.is_file() or not constants_path.is_file():
        return {
            ...
            "diagnosticRegion": START_REGION_BOUNDS,
            "features": [],
        }
...
def build_state(game_path: Path, user_dirs: list[Path], saves: list[Path], selected: Path) -> dict:
    ...
    return {
        ...
        "layers": [
            ...
            {"id": "water", "label": "Water", "available": bool(roads.get("waterCells")), "visible": True},
            {"id": "biomes", "label": "Biomes", "available": False, "visible": True},
            {
                "id": "anchors",
