...
function drawWater() {
  if (!layerVisibility.get("water")) return;
...
function drawDiagnosticRegion() {
  const bounds = state?.map?.diagnosticRegion;
  if (!bounds) return;
  const cell = state.map.cellSizeMeters;
  const bottomLeft = worldToScreen(bounds.xMin * cell, bounds.yMin * cell);
  const topRight = worldToScreen((bounds.xMax + 1) * cell, (bounds.yMax + 1) * cell);
  context.save();
  context.fillStyle = "rgb(159 207 130 / 4%)";
  context.strokeStyle = "rgb(159 207 130 / 55%)";
  context.lineWidth = 1;
  context.fillRect(
    bottomLeft.x,
    topRight.y,
...
function drawFeatures() {
  ...
  for (const feature of features) {
    if (!layerVisibility.get("anchors")) continue;
    ...
    if (feature.kind === "spawn") {
      ...
      context.fillStyle = "rgb(159 207 130 / 30%)";
      context.strokeStyle = "#9fcf82";
      context.lineWidth = 2;
      context.fillRect(
...
canvas.addEventListener("click", (event) => {
  ...
  const feature = [...(state.map.features || [])].reverse().find((item) => {
    if (!layerVisibility.get("anchors")) return false;
  ...
  document.getElementById("mapStatus").textContent =
    `${feature.title} · cell ${feature.cellX}, ${feature.cellY}. Visibility: generator-known; discovery ${feature.discoveryStatus} (${feature.discoverySource || "unconfirmed"}). ${feature.discoveryEvidence?.reason || ""} ${feature.details}${road}${tileDetails} Placement source: ${feature.source}.`;
