...
2. **Generate exact roads** - asks for Windows administrator approval,
   ...
   After capture, JC ScrapMap selects the save with the captured world seed,
   opens or refreshes its map, and leaves a clear result visible until Enter.
...
If a map is open on a different save, successful generation switches it to the
save matching the captured seed. If no matching save can be found, the map
explains that the captured roads belong to another world.
